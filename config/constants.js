import Vue from 'vue'

Vue.prototype.$constants = {
  baseURL: 'https://vue-fbd.9uylk5fy.xyz',
  headerBtnLink: 'https://www.zonapagos.com/t_fundacionbolivar',
  fbURL: 'https://fb.com',
  instaURL: 'https://instagram.com',
  linkedInURL: 'https://linkedin.com',
  twitterURL: 'https://twitter.com',
  youtubeURL: 'https://youtube.com',
  escuelaDeDirectores: {
    nuestrosBtnLink: 'javascript:void(0)',
    slides: [
      {
        imgURL: '/images/slide_01.jpg',
        text1: 'Unidad 1',
        text2: 'Método del caso',
        description:
          'Es un innovador proceso de aprendizaje y perfeccionamiento de capacidades directivas que se trabaja en tres niveles',
      },
      {
        imgURL: '/images/slide_01.jpg',
        text1: 'Unidad 2',
        text2: 'Método del caso',
        description:
          'Es un innovador proceso de aprendizaje y perfeccionamiento de capacidades directivas que se trabaja en tres niveles',
      },
      {
        imgURL: '/images/slide_01.jpg',
        text1: 'Unidad 3',
        text2: 'Método del caso',
        description:
          'Es un innovador proceso de aprendizaje y perfeccionamiento de capacidades directivas que se trabaja en tres niveles',
      },
      {
        imgURL: '/images/slide_01.jpg',
        text1: 'Unidad 4',
        text2: 'Método del caso',
        description:
          'Es un innovador proceso de aprendizaje y perfeccionamiento de capacidades directivas que se trabaja en tres niveles',
      },
    ],
    fechas:
      'La Escuela de directores la haremos el 18,19, 20,23,24,25 y 26 de Agosto en un horario de 8:00 am a 12:00.',
    actividades1:
      'El primer día se llevarán a cabo la inauguración, la introducción y la presentación a cargo de los directivos de la Fundación Bolívar Davivienda, y 1 sesión de trabajo.',
    actividades2:
      'Los seis días siguientes habrá entre 2 y 3 sesiones, para un total de 14. El último día celebraremos el cierre del seminario y se entregarán los certificados de asistencia.',
    tiempoDedicado:
      'Demandará una dedicación de 30 horas presenciales y aproximadamente 20 horas de estudio individual, para preparar los casos antes de cada sesión y lograr una mejor interacción y participación por parte de los asistentes.',
  },
  home: {
    slides: [
      {
        imgURL: '/images/seccion-inicio/carrusel1.jpg',
        title: 'Desarrollamos talento para construir país.',
        description:
          'Conozca todas nuestras Iniciativas y programas.',
      },
      {
        imgURL: '/images/seccion-inicio/carrusel2.jpg',
        title: 'Desarrollamos talento para construir país.',
        description:
          'Conozca todas nuestras Iniciativas y programas.',
      },
    ],
    desarrollamosTalento: 'javascript:void(0)',
    nuestrosProgramas: [
      {
        imgURL: '/images/seccion-inicio/home_programas_0006_aflora.jpg',
        title: 'Aflora',
        link: '/aflora',
        summary:
          'En Aflora impulsamos a las organizaciones sociales que quieren multiplicar su capacidad de servir, que buscan crecer para tener un mayor impacto, para que sean agentes de cambio sostenibles en el tiempo.',
      },
      {
        imgURL: '/images/seccion-inicio/home_programas_0005_becas.jpg',
        title: 'Becas al Talento',
        link: '/becas-al-talento',
        summary:
          'En Becas al Talento respaldamos el sueño de estudiantes destacados que tienen dificultades económicas para continuar sus carreras profesionales.<br>Apoyamos a aquellos que inspiran a otros a transformar su talento en oportunidades.',
      },
      {
        imgURL: '/images/seccion-inicio/home_programas_0004_cultivarte.jpg',
        title: 'Cultivarte',
        link: '/cultivarte',
        summary:
          'Cultivarte, un espacio donde niños, niñas y adolescentes descubren su talento y aprenden a usarlo pensando en su futuro. Los talentos de Cultivarte transforman sus realidades, las de sus familias e inspiran a otros en su entorno.',
      },
      {
        imgURL: '/images/seccion-inicio/home_programas_0003_emprende_pais.jpg',
        title: 'Emprende País',
        link: '/emprende-pais',
        summary:
          'Emprende País apoya el talento de líderes comprometidos con el desarrollo económico del país, brindando capacitaciones y herramientas para sus empresas.',
      },
      {
        imgURL: '/images/seccion-inicio/home_programas_0002_filarmonica.jpg',
        title: 'Filarmónica Joven de Colombia',
        link: '/filarmonica-joven-de-colombia',
        summary:
          'Filarmónica Joven de Colombia es una apuesta que promueve el talento musical de nuestro país para construir un proyecto de vida desde la pasión. Nuestros músicos llegan al mundo entero e inspiran a otros en Colombia a creer en el ritmo de sus sueños.',
      },
      {
        imgURL: '/images/seccion-inicio/home_programas_0001_inversion social.jpg',
        title: 'Inversión Social',
        link: '/inversion-social',
        summary:
          'En Inversión Social trabajamos por la transformación de historias de niños, niñas, adolescentes y mujeres que desde su rol como agentes de cambios en sus comunidades buscan soluciones sostenibles.',
      },
      {
        imgURL: '/images/seccion-inicio/home_programas_0000_voluntariado.jpg',
        title: 'Voluntariado Fundación Bolivar',
        link: '/voluntariado-fundacion-bolivar',
        summary:
          'En Voluntariado Familiar Bolívar unimos esfuerzos para que colaboradores, familiares y pensionados del Grupo sean una fuerza multiplicadora de solidaridad compartiendo conocimiento para transformar a otros porque ayudar, inspira.',
      },
    ],
    elCambio: 'javascript:void(0)',
    foundationNews: [
      {
        smallTitle: 'Título de la noticia',
        bigTitle: 'Título de la noticia, <br />puede bajar a segunda línea',
        imgURL: '/images/new_img_a.jpg',
        summary:
          'Conveniently embrace resource sucking meta-services via scalabe.',
        date: 'September 16 - 2021',
        tags: [
          {
            title: 'Categoría',
            url: 'javascript:void(0)',
          },
          {
            title: '#Cultivarte',
            url: 'javascript:void(0)',
          },
          {
            title: '#Trabajo',
            url: 'javascript:void(0)',
          },
        ],
      },
      {
        smallTitle: 'Título de la noticia',
        bigTitle: 'Título de la noticia, <br />puede bajar a segunda línea',
        imgURL: '/images/new_img_b.jpg',
        summary:
          'Conveniently embrace resource sucking meta-services via scalabe.',
        date: 'September 16 - 2021',
        tags: [
          {
            title: 'Categoría',
            url: 'javascript:void(0)',
          },
          {
            title: '#Cultivarte',
            url: 'javascript:void(0)',
          },
          {
            title: '#Trabajo',
            url: 'javascript:void(0)',
          },
        ],
      },
      {
        smallTitle: 'Título de la noticia',
        bigTitle: 'Título de la noticia, <br />puede bajar a segunda línea',
        imgURL: '/images/new_img_c.jpg',
        summary:
          'Conveniently embrace resource sucking meta-services via scalabe.',
        date: 'September 16 - 2021',
        tags: [
          {
            title: 'Categoría',
            url: 'javascript:void(0)',
          },
          {
            title: '#Cultivarte',
            url: 'javascript:void(0)',
          },
          {
            title: '#Trabajo',
            url: 'javascript:void(0)',
          },
        ],
      },
    ],
    meta: {
      title: 'Fundación Bolivar Davivienda',
      metaTitle: 'Fundación Bolivar Davivienda',
      metaDescription: 'Nos inspiramos en el talento de personas, organizaciones y empresas para construir juntos un mejor país.',
      ogTitle: 'Fundación Bolivar Davivienda',
      ogDescription: 'Nos inspiramos en el talento de personas, organizaciones y empresas para construir juntos un mejor país.',
      ogImage: '/images/seccion-inicio/home_og.jpg',
    },
  },
  laFundacion: {
    tabs: [
      {
        title: 'Nuestra Misión',
        description:
          '<p>Como expresión del compromiso social del Grupo Bolívar, apoyamos y potenciamos proyectos transformadores y de alto impacto que generan capacidades en personas, comunidades y organizaciones, para construir una sociedad más justa, equitativa e innovadora.</p>',
        tags: [
          {
            title: 'Conoce Nuestros Estatutos',
            url: 'javascript:void(0)',
          },
          {
            title: 'Código de Gobierno Corporativo',
            url: 'javascript:void(0)',
          },
          {
            title: 'Reglamento Ejecutivo',
            url: 'javascript:void(0)',
          },
        ],
      },
      {
        title: 'Nuestra Visión',
        description:
          '<p>La Fundación Bolívar Davivienda será reconocida como una fundación líder en los lugares donde está presente el grupo, por el impacto de sus programas en el logro de transformaciones sociales efectivas y sostenibles.</p>',
        tags: [
          {
            title: 'Conoce Nuestros Estatutos',
            link: 'javascript:void(0)',
          },
          {
            title: 'Código de Gobierno Corporativo',
            link: 'javascript:void(0)',
          },
          {
            title: 'Reglamento Ejecutivo',
            link: 'javascript:void(0)',
          },
        ],
      },
      {
        title: 'Gobierno Corporativo',
        description:
          '<p>En la Fundación Bolívar Davivienda contamos con un Sistema de Buen Gobierno en armonía con los estatutos, a través del cual se definen e identifican las funciones, naturaleza y razón de ser de cada uno de nuestros órganos de Gobierno Corporativo (Consejo Directivo, Consejo Ejecutivo, Director Ejecutivo y Revisor Fiscal), asignando las responsabilidades y obligaciones que permiten el cumplimiento de las metas fundacionales y la adecuada atención a las necesidades de los diferentes grupos de interés.</p>' +
          '<p>Lo anterior, se complementa con la promoción de competencias profesionales y del comportamiento, que priman como elementos fundamentales del desarrollo de la institución entre fundadores, administradores y colaboradores, donde el respeto hacia los valores éticos se convierte en un activo imprescindible que guía el actuar cotidiano de nuestra organización.</p>' +
          '<p>Los miembros de los órganos de dirección y administración desempeñan tales cargos en atención a sus altas condiciones morales y profesionales.</p>',
        tags: [
          {
            title: 'Conoce Nuestros Estatutos',
            link: 'javascript:void(0)',
          },
          {
            title: 'Código de Gobierno Corporativo',
            link: 'javascript:void(0)',
          },
          {
            title: 'Reglamento Ejecutivo',
            link: 'javascript:void(0)',
          },
        ],
      },
    ],
    meta: {
      title: 'La Fundación',
      metaTitle: 'La Fundación',
      metaDescription: 'Somos el brazo social del Grupo Empresarial Bolívar, con más de 75 años de trayectoria, construyendo confianza y una sólida reputación en diversos sectores del mercado colombiano.',
      ogTitle: 'La Fundación',
      ogDescription: 'Somos el brazo social del Grupo Empresarial Bolívar, con más de 75 años de trayectoria, construyendo confianza y una sólida reputación en diversos sectores del mercado colombiano.',
      ogImage: '/images/seccion-la-fundacion/lafundacion_og.jpg',
    },
  },
  aflora: {
    desarrollamosTalento: 'javascript:void(0)',
    meta: {
      title: 'Aflora',
      metaTitle: 'Aflora',
      metaDescription: 'Aflora',
      ogTitle: 'Aflora',
      ogDescription: 'Aflora',
      ogImage: '/images/programa-aflora/3-desarrollamos-talento/aflora_desarrollamostalento_og.jpg',
    },
  },
  afloraConozcanos: {
    testimonios: [
      {
        title: 'Carolina Escalona - Presidenta Fundación Rafael Escalona',
        description:
          'Con Aflora pudimos realizar muchos cambios internos y reconocer qué nos hace falta para llegar a ser una organización autosostenible y con visión.',
      },
      {
        title: 'Marian Tertel - Coordinadora en la Fundación Sole Colombia',
        description:
          'Gracias a Aflora pudimos entender dónde estábamos y hacia dónde teníamos que proyectarnos para cumplir todos nuestros objetivos.',
      },
      {
        title:
          'Isabel Cristina Rengifo - Representante legal de Fundación Mis Corazones Alegres',
        description:
          'Con los ciclos de Aflora logramos armar bien los proyectos y fortalecer los  procesos internos de la fundación',
      },
      {
        title:
          'Isabel Cristina Rengifo - Representante legal de Fundación Mis Corazones Alegres',
        description:
          'Con los ciclos de Aflora logramos armar bien los proyectos y fortalecer los  procesos internos de la fundación',
      },
    ],
    meta: {
      title: 'Conózcanos',
      metaTitle: 'Conózcanos',
      metaDescription: 'Conózcanos Description',
      ogTitle: 'Conózcanos',
      ogDescription: 'Conózcanos',
      ogImage: '/images/fundacion_bolivar_davivenda.png',
    },
  },
  afloraDesarrollamosTalento: {
    quieroConocerMas: 'javascript:void(0)',
    nivelesLink: 'javascript:void(0)',
    escuelaLink: 'javascript:void(0)',
    meta: {
      title: 'Desarrollamos talento',
      metaTitle: 'Desarrollamos talento',
      metaDescription: 'Desarrollamos talento Description',
      ogTitle: 'Desarrollamos talento',
      ogDescription: 'Desarrollamos talento',
      ogImage: '/images/fundacion_bolivar_davivenda.png',
    },
  },
  afloraDesarrollamosTalentoCiclosDeFormacion: {
    meta: {
      title: 'Desarrollamos talento',
      metaTitle: 'Desarrollamos talento',
      metaDescription: 'Desarrollamos talento Description',
      ogTitle: 'Desarrollamos talento',
      ogDescription: 'Desarrollamos talento',
      ogImage: '/images/fundacion_bolivar_davivenda.png',
    },
  },
  afloraAutodiagnostico: {
    importantInformations: [
      'Debe contar con 1 hora de su tiempo aproximadamente, para diligenciar todo el formulario.',
      'El formulario debe ser diligenciado por el líder de la organización, junto con su equipo para clasificar en uno de los 4 niveles.',
      'Es fundamental que todas las respuestas sean contestadas con absoluta sinceridad sobre el estado real de la organización, con el fin de encontrar una ruta que realmente se adecue a las necesidades',
      'Una vez finalizado el formulario, el sistema automáticamente clasificará la organización con el fin de brindarles la mejor ruta de fortalecimiento.',
    ],
    meta: {
      title: 'Aflora - Autodiagnóstico',
      metaTitle: 'Aflora - Autodiagnóstico',
      metaDescription: 'Autodiagnóstico Description',
      ogTitle: 'Autodiagnóstico',
      ogDescription: 'Autodiagnóstico',
      ogImage: '/images/programa-aflora/4-autodiagnostico/aflora_autodiagnostico_og.jpg',
    },
  },
  afloraInscripciones: {
    reglamentoLink: 'javascript:void(0)',
    inscribaseLink:
      'https://fundacionbolivardaviviendaorg.force.com/comunidadfbd/s/?language=es_CO&programa=Aflora&grupo=Estudiantes+de+escuelas',
    meta: {
      title: 'Aflora - Inscripciones',
      metaTitle: 'Afora - Inscripciones',
      metaDescription: 'Inscripciones Description',
      ogTitle: 'Inscripciones',
      ogDescription: 'Inscripciones',
      ogImage: '/images/programa-aflora/6-inscripciones/aflora_inscripciones_og.jpg',
    },
  },
  afloraNuestrasOrganizaciones: {
    foundations: [
      {
        title: 'Fundación Amigos de Amagá',
        address: 'Carrera 43 a calle 18 sur 13, Amagá, Antioquia, Colombia',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Formalización',
        contactoEMail: '',
        contactoWebsite: '',
        departamento: 'Antioquia'
      },
      {
        title: 'Corporación para el Desarrollo Integral de Las Inteligencias CORCEDIN',
        address: 'Calle 99 N° 107-20, Apartadó, Antioquia, Colombia.',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Formalización',
        contactoEMail: 'monicagutierrez011@gmail.com',
        contactoWebsite: '',
        departamento: 'Antioquia'
      },
      {
        title: 'Asociación de Egresados de la Universidad de Antioquia Seccional Bajo Cauca',
        address: 'Cl 22 # 20 - 84, Caucasia, Antioquia, Colombia',
        lineaDeGestion: 'No clasificado',
        nivel: 'Formalización',
        contactoEMail: 'janaya1973@gmail.com',
        contactoWebsite: '',
        departamento: 'Antioquia'
      },
      {
        title: 'Corporación Escuela de Música Jardín',
        address: 'Carrera 2 # 8-76, Jardín, Antioquia, Colombia',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Formalización',
        contactoEMail: 'corpomusica_jardin@yahoo.es',
        contactoWebsite: '',
        departamento: 'Antioquia'
      },
      {
        title: 'Asociación Red de Biocomercio Mercado Justo y Negocios Solidarios',
        address: 'Cl. 22 #3842, Marinilla, Antioquia, Colombia.',
        lineaDeGestion: 'No clasificado',
        nivel: 'Formalización',
        contactoEMail: 'gerencia@redbiocomercio.org',
        contactoWebsite: '',
        departamento: 'Antioquia'
      },
      {
        title: 'Asociación Colombiana de Psiconeuroinmunoendocrinología ACPNIE',
        address: 'Calle 49 # 50-21, Medellín, Antioquia, Colombia',
        lineaDeGestion: 'Educación e Investigación',
        nivel: 'Formalización',
        contactoEMail: 'presidencia@pniecolombia.org',
        contactoWebsite: 'http://www.pniecolombia.org/',
        departamento: 'Antioquia'
      },
      {
        title: 'Fundación La Casita Montessori',
        address: 'Cra 25 # 9A Sur-100, Medellín, Antioquía, Colombia',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Despegue',
        contactoEMail: 'administracion@mambq.org',
        contactoWebsite: 'https://www.mambq.org/',
        departamento: 'Antioquia'
      },
      {
        title: 'Fundación Grupo Internacional de Paz y Desarrollo',
        address: 'Cra 43B # 11-59 Oficina 201,MEDELLÍN,ANTIOQUIA,COLOMBIA',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Formalización',
        contactoEMail: '',
        contactoWebsite: '',
        departamento: 'Antioquia'
      },
      {
        title: 'Fundación Nacional de Trasplantados',
        address: 'Clle 48 C No 66 52, Medellín, Antioquia, Colombia',
        lineaDeGestion: 'Servicios Sociales',
        nivel: 'Formalización',
        contactoEMail: 'ana.c1963@hotmail.com',
        contactoWebsite: '',
        departamento: 'Antioquia'
      },
      {
        title: 'Fundación Mis Corazones Alegres',
        address: 'Carrera 50 # 60 - 26, Medellín, Antioquia, Colombia',
        lineaDeGestion: 'Servicios Sociales',
        nivel: 'Formalización',
        contactoEMail: 'direccion@panypanela.com',
        contactoWebsite: '',
        departamento: 'Antioquia'
      },
      {
        title: 'Fundación Fútbol para el Futuro',
        address: 'Calle 48 #48-14, Medellín, Antioquia, Colombia',
        lineaDeGestion: 'Servicios Sociales',
        nivel: 'Formalización',
        contactoEMail: 'adrianalievano@fundacioninti.org',
        contactoWebsite: 'https://www.fundacioninti.org/',
        departamento: 'Antioquia'
      },
      {
        title: 'Comité Privado De Asistencia A La Niñez Pan',
        address: 'Calle 49 No 19 AA 99,MEDELLÍN,ANTIOQUIA,COLOMBIA',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Formalización',
        contactoEMail: '',
        contactoWebsite: '',
        departamento: 'Antioquia'
      },
      {
        title: 'Fundación SACIAR',
        address: 'Cra. 50 #25-261, Medellín, Antioquia, Colombia',
        lineaDeGestion: 'Servicios Sociales',
        nivel: 'Despegue',
        contactoEMail: 'info@semillayfruto.org.co',
        contactoWebsite: 'https://www.semillayfruto.org.co/',
        departamento: 'Antioquia'
      },
      {
        title: 'Fundación Ximena Rico Llano',
        address: 'Calle 47 No. 36A-96, Medellín, Antioquia, Colombia',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Formalización',
        contactoEMail: 'leitomenji@gmail.com',
        contactoWebsite: '',
        departamento: 'Antioquia'
      },
      {
        title: 'Corporación Grupo Trópico Diverso',
        address: 'Carrera 65 No. 31 - 34, Medellín, Antioquia, Colombia',
        lineaDeGestion: 'Medio Ambiente',
        nivel: 'Formalización',
        contactoEMail: 'grupotropicodiverso@gmail.com',
        contactoWebsite: '',
        departamento: 'Antioquia'
      },
      {
        title: 'Corporación Gimnasio Nuevo Milenio',
        address: 'Calle 68 A N° 30-4, Medellín, Antioquia, Colombia.',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Formalización',
        contactoEMail: 'corginumil@hotmail.com',
        contactoWebsite: 'http://www.gimnasionuevomileniofc.es.tl/',
        departamento: 'Antioquia'
      },
      {
        title: 'Corporación Entreviñetas',
        address: 'Carrera 41 #9.51, Medellín, Antioquia,Colombia',
        lineaDeGestion: 'No clasificado',
        nivel: 'Formalización',
        contactoEMail: 'daniel@entrevinetas.com',
        contactoWebsite: 'http://www.entrevinetas.com/',
        departamento: 'Antioquia'
      },
      {
        title: 'Corporación el Balcón de los Artistas',
        address: 'CARRERA 31 A 71 A 30, MEDELLIN, ANTIOQUIA, COLOMBIA',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Formalización',
        contactoEMail: 'info@balcondelosartistas.org',
        contactoWebsite: 'http://www.elbalcon.org/web/',
        departamento: 'Antioquia'
      },
      {
        title: 'Corporación Dulazar Provivienda Digna',
        address: 'Calle 49B N° 63 – 21, Medellín, Antioquia, Colombia.',
        lineaDeGestion: 'Desarrollo y Vivienda',
        nivel: 'Formalización',
        contactoEMail: 'gerencia@corporaciondulazar.com.co',
        contactoWebsite: 'http://www.corporaciondulazar.org/',
        departamento: 'Antioquia'
      },
      {
        title: 'Corporación Cultural Canchimalos',
        address: 'Calle 47 # 80 - 37, Medellín, Antioquia, Colombia',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Formalización',
        contactoEMail: 'canchimaloscorporacion@gmail.com',
        contactoWebsite: 'http://canchimalos.co/',
        departamento: 'Antioquia'
      },
      {
        title: 'Corporación Creaciones Ecovida',
        address: 'Cll 62D 132 - 55, Medellín, Antioquia, Colombia',
        lineaDeGestion: 'No clasificado',
        nivel: 'Formalización',
        contactoEMail: 'herson3372@hotmail.com',
        contactoWebsite: '',
        departamento: 'Antioquia'
      },
      {
        title: 'Corporación Comunitaria Monseñor Romero',
        address: 'Diagonal 42F# 36 C 115, Medellín, Antioquia, Colombia',
        lineaDeGestion: 'Desarrollo y Vivienda',
        nivel: 'Formalización',
        contactoEMail: 'corvideco.monseromero@gmail.com',
        contactoWebsite: '',
        departamento: 'Antioquia'
      },
      {
        title: 'Corporación Centro de Atención a la Familia Fabiola Tamayo Atehortua',
        address: 'Calle 10 # 5 - 13, Medellín, Antioquia, Colombia',
        lineaDeGestion: 'No clasificado',
        nivel: 'Formalización',
        contactoEMail: 'gloriaedh@hotmail.com',
        contactoWebsite: '',
        departamento: 'Antioquia'
      },
      {
        title: 'Corporación Casa Taller Artesas',
        address: 'Cl. 54a #77b26, Medellín, Antioquia',
        lineaDeGestion: 'No clasificado',
        nivel: 'Formalización',
        contactoEMail: 'corporacion.artesas@hotmail.com',
        contactoWebsite: '',
        departamento: 'Antioquia'
      },
      {
        title: 'Corporación Casa Mía',
        address: 'Cra. 76b #10852, Medellín, Antioquia',
        lineaDeGestion: 'Negocios y Asociaciones Profesionales',
        nivel: 'Formalización',
        contactoEMail: 'gestion@corpcasamia.org',
        contactoWebsite: '',
        departamento: 'Antioquia'
      },
      {
        title: 'Corporación Camino a los Sueños del Mañana',
        address: 'Cra 82 # 20 A - 00 Medellín, Antioquia, Colombia',
        lineaDeGestion: 'Servicios Sociales',
        nivel: 'Formalización',
        contactoEMail: 'dissmanhj@hotmail.com',
        contactoWebsite: '',
        departamento: 'Antioquia'
      },
      {
        title: 'Corporación Artística y Cultural ACROBATIC',
        address: 'Cra 73 N 96-105, Medellín, Antioquia, Colombia',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Formalización',
        contactoEMail: 'acionacrobatic@hotmail.com',
        contactoWebsite: 'http://www.corporacionacrobatic.org/',
        departamento: 'Antioquia'
      },
      {
        title: 'Corporación Antioqueña del Medio Ambiente',
        address: 'Cra 48 N° 59 - 52, Medellín, Antioquia, Colombia',
        lineaDeGestion: 'Medio Ambiente',
        nivel: 'Formalización',
        contactoEMail: 'corama2012@gmail.com',
        contactoWebsite: 'http://coramamedellin.wixsite.com/corpoantioquena',
        departamento: 'Antioquia'
      },
      {
        title: 'Corporación al Derecho Colombia',
        address: 'Calle 77 E # 88 - 18, Medellín, Antioquia, Colombia',
        lineaDeGestion: 'Negocios y Asociaciones Profesionales',
        nivel: 'Formalización',
        contactoEMail: 'corpoalderecho@hotmail.com',
        contactoWebsite: '',
        departamento: 'Antioquia'
      },
      {
        title: 'Asociación de Vivienda Comunitaria Villa Francisca',
        address: 'Diagonal 81B No.76 123, Medellín, Antioquia, Colombia',
        lineaDeGestion: 'Desarrollo y Vivienda',
        nivel: 'Formalización',
        contactoEMail: 'janetthrios@gmail.com',
        contactoWebsite: '',
        departamento: 'Antioquia'
      },
      {
        title: 'Asociación de Pedagogos Reeducadores Egresados de la Fundación Universitaria Luis Amigó ASPERLA',
        address: 'Calle 64 # 50-44, Medellín, Colombia',
        lineaDeGestion: 'Servicios Sociales',
        nivel: 'Formalización',
        contactoEMail: 'comunicaciones@hogarasperla.org',
        contactoWebsite: 'http://www.asperla.org/',
        departamento: 'Antioquia'
      },
      {
        title: 'Asociación Cívica Comunitaria Barrio los Monjes',
        address: 'Calle 55A # 85J - 10, Medellin, Antioquia',
        lineaDeGestion: 'Servicios Sociales',
        nivel: 'Formalización',
        contactoEMail: 'acivicom@gmail.com',
        contactoWebsite: 'http://acivicom.webnode.es/',
        departamento: 'Antioquia'
      },
      {
        title: 'Asociación Antioqueña para la Cultura',
        address: 'Cra. 25 # 50-38 Medellín, Antioquia, Colombia',
        lineaDeGestion: 'Medio Ambiente',
        nivel: 'Formalización',
        contactoEMail: 'asac.medellin.1977@gmail.com',
        contactoWebsite: '',
        departamento: 'Antioquia'
      },
      {
        title: 'Centro de Cultura Creativa CECREA',
        address: 'Clle 85#42G-23, Puerto Colombia, Antioquia, Colombia',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Formalización',
        contactoEMail: 'anibaltobon@gmail.com',
        contactoWebsite: '',
        departamento: 'Antioquia'
      },
      {
        title: 'Asociación de Personas en Situación de Discapacidad de San Andrés ADISA',
        address: 'Calle Ayacucho No. 28, Colombia',
        lineaDeGestion: 'Servicios Sociales',
        nivel: 'Formalización',
        contactoEMail: 'alrp10@hotmail.com',
        contactoWebsite: '',
        departamento: 'Antioquia'
      },
      {
        title: 'Corporación Escuela de Música Jorge Robledo Ortiz de Santa Fé de Antioquia',
        address: 'CALLE 10 # 7 - 09, Santa fe de Antioquia, Colombia.',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Crecimiento',
        contactoEMail: 'escuelamusicajro@yahoo.es',
        contactoWebsite: '',
        departamento: 'Antioquia'
      },
      {
        title: 'Asociación Grupo Ecológico Semillas Del Futuro',
        address: ' Calle 20 N° 18- 45 piso 2\n',
        lineaDeGestion: 'Cultura y recreación',
        nivel: 'Formalización',
        contactoEMail: 'consueloyepes439@hotmail.com',
        contactoWebsite: '',
        departamento: 'Antioquia'
      },
      {
        title: 'Fundación Ludovica',
        address: 'Amberes Cra 44 30-93 Piso 3, Barranquilla, Atlántico, Colombia',
        lineaDeGestion: 'Servicios Sociales',
        nivel: 'Formalización',
        contactoEMail: 'direccion@fnvpt.org',
        contactoWebsite: 'https://www.fnvpt.org/',
        departamento: 'Atántico'
      },
      {
        title: 'Fundación para el Desarrollo Humano Comunitario FUNDEHUMAC',
        address: 'Calle 22 No 20-49, Barranquilla, Atlántico, Colombia',
        lineaDeGestion: 'Servicios Sociales',
        nivel: 'Formalización',
        contactoEMail: 'fundacionpresentes@gmail.com',
        contactoWebsite: 'http://www.ucppresentes.com/',
        departamento: 'Atántico'
      },
      {
        title: 'Corporación Cultural Nave',
        address: 'Cl 47 A # 4 Sur 11, Barranquilla, Atlántico, Colombia',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Formalización',
        contactoEMail: 'corporacionnave@gmail.com',
        contactoWebsite: 'http://culturalnave.wixsite.com/cultural-nave',
        departamento: 'Atántico'
      },
      {
        title: 'Corporación Centro de Educación Especial y Rehabilitación CEER',
        address: 'Cl. 75 #48-25, Barranquilla, Atlántico, Colombia',
        lineaDeGestion: 'Servicios Sociales',
        nivel: 'Formalización',
        contactoEMail: 'ritadelacruzpicalua@gmail.com',
        contactoWebsite: '',
        departamento: 'Atántico'
      },
      {
        title: 'Corporación Biólogos por Colombia',
        address: 'Calle 106 N° 90 - 109, Barranquilla, Antlántico, Colombia',
        lineaDeGestion: 'Medio Ambiente',
        nivel: 'Formalización',
        contactoEMail: 'direccion@biologosporcolombia.org',
        contactoWebsite: 'http://www.biologosporcolombia.org/',
        departamento: 'Atántico'
      },
      {
        title: 'CODAFE',
        address: 'Calle 5 4 - 42, Barranquilla, Atlántico, Colombia.',
        lineaDeGestion: 'No clasificado',
        nivel: 'Formalización',
        contactoEMail: 'codafe@metrotel.net.co',
        contactoWebsite: 'https://codafe.wordpress.com/',
        departamento: 'Atántico'
      },
      {
        title: 'Club de Leones de Barranquilla Monarcas',
        address: 'Calle Santa Fe Mz 26 Lt 50, Barranquilla, Atlántico, Colombia.',
        lineaDeGestion: 'No clasificado',
        nivel: 'Despegue',
        contactoEMail: 'centroeducativoceral2011@hotmail.com',
        contactoWebsite: '',
        departamento: 'Atántico'
      },
      {
        title: 'Asociación Red Juvenil del Suroccidente Barranquilla',
        address: 'Calle 73 E # 9J 1-15, Barranquilla, Atlántico.',
        lineaDeGestion: 'Desarrollo y Vivienda',
        nivel: 'Formalización',
        contactoEMail: 'libardodiago2012@gmail.com',
        contactoWebsite: 'https://redjuvenilbarranquilla.com/',
        departamento: 'Atántico'
      },
      {
        title: 'Asociación Red Ambiental Solidaria San Antonio de Prado',
        address: 'Cra 68 No. 64Sur 33 Vereda San José, Barranquilla, Atlántico.',
        lineaDeGestion: 'No clasificado',
        nivel: 'Formalización',
        contactoEMail: 'arasolesarasoles@gmail.com',
        contactoWebsite: 'http://www.arasoles.org/',
        departamento: 'Atántico'
      },
      {
        title: 'Asociación Promotora de la Cultura en el Caribe ASCARIBE',
        address: 'Cra 53 # 75-87, Barranquilla, Atlántico, Colombia',
        lineaDeGestion: 'Servicios Sociales',
        nivel: 'Formalización',
        contactoEMail: 'edithmarmo@yahoo.com',
        contactoWebsite: '',
        departamento: 'Atántico'
      },
      {
        title: 'Asociación Lila Mujer',
        address: 'Calle 116A #26 G 03, Barranquilla, Atlantico, Colombia',
        lineaDeGestion: 'Servicios Sociales',
        nivel: 'Formalización',
        contactoEMail: 'lilamujer@gmail.com',
        contactoWebsite: '',
        departamento: 'Atántico'
      },
      {
        title: 'Asociación de Profesionales Unidos por el Desarrollo Etnocultural Social y Ambiental de Colombia',
        address: 'Carrera 44 N° 40 - 20, Barranquilla, Atlantico, Colombia',
        lineaDeGestion: 'Negocios y Asociaciones Profesionales',
        nivel: 'Formalización',
        contactoEMail: 'asoproudesco@gmail.com',
        contactoWebsite: 'http://asoproudescojuridi.wixsite.com/juridica',
        departamento: 'Atántico'
      },
      {
        title: 'Apanexda',
        address: 'Cra. 45 #82-93, Barranquilla, Atlántico, Colombia',
        lineaDeGestion: 'Educación e Investigación',
        nivel: 'Formalización',
        contactoEMail: 'mazcoingenieros@hotmail.com',
        contactoWebsite: 'http://www.actiweb.es/apanexda/',
        departamento: 'Atántico'
      },
      {
        title: 'Fundación Comunidad más Generación COMASGE',
        address: 'Soledad, Atlántico',
        lineaDeGestion: 'Servicios Sociales',
        nivel: 'Formalización',
        contactoEMail: 'doctoraclown@doctoraclown.org',
        contactoWebsite: 'https://www.doctoraclown.org/',
        departamento: 'Atántico'
      },
      {
        title: 'Fundación Cresiendo',
        address: 'Cra. 74a #7221, Cartagena, Bolivar',
        lineaDeGestion: 'Educación e Investigación',
        nivel: 'Formalización',
        contactoEMail: 'maria.jaramillo@fundacioncresiendo.org',
        contactoWebsite: 'https://www.fundacioncresiendo.org/',
        departamento: 'Bolívar'
      },
      {
        title: 'Fundación Indufrial',
        address: '39, Dg. 21 #49, Cartagena, Bolívar',
        lineaDeGestion: 'Desarrollo y Vivienda',
        nivel: 'Crecimiento',
        contactoEMail: 'somosludovica@gmail.com',
        contactoWebsite: 'https://www.fundacionludovica.tumblr.com/',
        departamento: 'Bolívar'
      },
      {
        title: 'Asociación la Casita Hogar de la Joven',
        address: 'Calle 42 No. 28-41, Cartagena, Bolivar, Colombia',
        lineaDeGestion: 'Servicios Sociales',
        nivel: 'Despegue',
        contactoEMail: 'ardilaedilma@hotmail.com',
        contactoWebsite: '',
        departamento: 'Bolívar'
      },
      {
        title: 'Asociación de Coordinadoras por la Dignidad y el Respeto del Adulto Mayor ASOCOORAM',
        address: 'Calle 47·2-69, Cartagena, Bolivar, Colombia',
        lineaDeGestion: 'No clasificado',
        nivel: 'Formalización',
        contactoEMail: 'sady302008@hotmail.com',
        contactoWebsite: '',
        departamento: 'Bolívar'
      },
      {
        title: 'Fundación Milagro de Amor',
        address: '',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Formalización',
        contactoEMail: 'fundacionmilagrodeamor2002@gmail.com',
        contactoWebsite: '',
        departamento: 'Boyacá'
      },
      {
        title: 'Corporación el Faro',
        address: 'Calle 49 C N° 34 C 28, Manizales, Caldas, Colombia',
        lineaDeGestion: 'No clasificado',
        nivel: 'Despegue',
        contactoEMail: 'dilo13831@gmail.com',
        contactoWebsite: '',
        departamento: 'Caldas'
      },
      {
        title: 'Corporación de la Sagrada Familia',
        address: 'Calle 60 #23A - 25, Manizales, Caldas, Colombia',
        lineaDeGestion: 'Servicios Sociales',
        nivel: 'Formalización',
        contactoEMail: 'corpofami@hotmail.com',
        contactoWebsite: 'https://corpofami.wixsite.com/sagrada-familia',
        departamento: 'Caldas'
      },
      {
        title: 'Centro de Desarrollo Comunitario Versalles',
        address: 'Carrera 22 No 46 -19, Manizales, Caldas, Colombia',
        lineaDeGestion: 'Servicios Sociales',
        nivel: 'Formalización',
        contactoEMail: 'centroversalles2@gmail.com',
        contactoWebsite: 'http://www.centroversalles.org/',
        departamento: 'Caldas'
      },
      {
        title: 'Asociación Damas Voluntarias',
        address: 'Carrera 24 N° 22 - 51, Manizales, Caldas, Colombia',
        lineaDeGestion: 'Servicios Sociales',
        nivel: 'Formalización',
        contactoEMail: 'damasvoluntariasvicentinasmles@gmail.com',
        contactoWebsite: 'http://siembrasocial.com/',
        departamento: 'Caldas'
      },
      {
        title: 'Acal Asociación Cultural Caldense',
        address: 'Cra 23 #62-74, Manizales, Caldas, Colombia',
        lineaDeGestion: 'Servicios Sociales',
        nivel: 'Formalización',
        contactoEMail: 'beatrizgomezdejaramillo@gmail.com',
        contactoWebsite: '',
        departamento: 'Caldas'
      },
      {
        title: 'Club Deportivo, Artístico y Cultural Semillas del Micay',
        address: '',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Formalización',
        contactoEMail: 'semillasmicay@gmail.com',
        contactoWebsite: '',
        departamento: 'Cauca'
      },
      {
        title: 'Fundación Niños de la Esperanza ESAL Colombia',
        address: 'Carrera 10 65 35 Oficina 601, Bogotá D.C, Colombia',
        lineaDeGestion: 'Educación e Investigación',
        nivel: 'Formalización',
        contactoEMail: 'patyferreira.direcciongeneral@icalcolombia.org',
        contactoWebsite: 'https://www.icalcolombia.org/',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Ar Fundación',
        address: ' Calle 113 No. 7-80 Piso 18\n',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Formalización',
        contactoEMail: 'dmora@arfundacion.org',
        contactoWebsite: '',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Talento 21',
        address: ' Cra. 3 A No. 26 B 10',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Formalización',
        contactoEMail: 'maritzalopez@talento21.com.co',
        contactoWebsite: '',
        departamento: 'Cundinamarca'
      },
      {
        title: 'World Vision International',
        address: 'Calle 10 Norte Numero 17 A 19, Bogotá D.C, Colombia',
        lineaDeGestion: 'Internacional',
        nivel: 'Formalización',
        contactoEMail: '',
        contactoWebsite: 'https://www.worldvision.co/',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Fundación Operación Sonrisa Colombia',
        address: 'Calle 90 No. 19C - 41,BOGOTÁ D.C.,CUNDINAMARCA,COLOMBIA',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Formalización',
        contactoEMail: '',
        contactoWebsite: '',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Fundación Libérate',
        address: 'Trasversal 77#166-40, Bogotá D.C, Colombia',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Formalización',
        contactoEMail: 'claudiat@escueladedanzanijinsky.com.co',
        contactoWebsite: '',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Prana Incubadora de Empresas Culturales',
        address: 'Carrera 15 No. 97-40 Oficina 503, Bogotá D.C, Colombia',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Crecimiento',
        contactoEMail: 'pranainc@gmail.com',
        contactoWebsite: 'https://www.pranaincubadora.org/',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Fundación Guaicaramo',
        address: 'Carrera 73 59-12 Sur C.C. Metrosur Local 211, Bogotá D.C, Colombia',
        lineaDeGestion: 'No clasificado',
        nivel: 'Formalización',
        contactoEMail: 'sibarra@funguaicaramo.org',
        contactoWebsite: 'https://www.funguaicaramo.org/',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Fundación CEPYTIN',
        address: 'Carrera 73#7c-39, Bogotá D.C, Colombia',
        lineaDeGestion: 'Educación e Investigación',
        nivel: 'Despegue',
        contactoEMail: 'percompu@hotmail.com',
        contactoWebsite: 'https://www.percomputo.org/',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Fundación Red Sonrisas',
        address: 'Calle 130a 101 c 08',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Formalización',
        contactoEMail: '',
        contactoWebsite: 'https://fenixfc.org/',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Fundación Misión Vida',
        address: 'Calle 1 F No. 31 - 11\n',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Formalización',
        contactoEMail: '',
        contactoWebsite: 'https://misionvida@vidaenaccionicc.org/',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Fundación Bogotá Es Teatro',
        address: 'Cra 35 #53a 89 apt 402,BOGOTÁ D.C.,CUNDINAMARCA,COLOMBIA',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Formalización',
        contactoEMail: '',
        contactoWebsite: '',
        departamento: 'Cundinamarca'
      },
      {
        title: 'ONG Habider Internacional',
        address: ' CARRERA 89 B No. 75 92',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Formalización',
        contactoEMail: 'mirtova123@gmail.com',
        contactoWebsite: '',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Fundación Huellas Verdes Colombia',
        address: 'Calle 152 No 12C-12, Bogotá D.C, Colombia',
        lineaDeGestion: 'No clasificado',
        nivel: 'Despegue',
        contactoEMail: 'lider.paz@hotmail.com',
        contactoWebsite: 'https://www.lideresconstructoresdepaz.org/',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Fundación Proyecto de Vida',
        address: 'Cra 11 No. 94-47, Bogotá D.C, Colombia',
        lineaDeGestion: 'Servicios Sociales',
        nivel: 'Formalización',
        contactoEMail: 'fundacionsocialcasamayor@hotmail.com',
        contactoWebsite: '',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Fundación Kolping',
        address: ' Carrera 16 N° 35-41 Teusaquillo\n',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Formalización',
        contactoEMail: 'Direccionnacional@kolpingcolombia.com',
        contactoWebsite: '',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Fundación Niñas de Luz',
        address: 'Calle 127 B Bis 53 A 28 Apto.415, Bogotá D.C, Colombia',
        lineaDeGestion: 'Medio Ambiente',
        nivel: 'Formalización',
        contactoEMail: 'mays.fundacion@gmail.com',
        contactoWebsite: '',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Fundación del Quemado',
        address: 'Calle 128B # 59B-20, Bogotá D.C, Colombia',
        lineaDeGestion: 'No clasificado',
        nivel: 'Despegue',
        contactoEMail: 'reysalomon05@misena.edu.co',
        contactoWebsite: 'https://www.fevolucionarco.weebly.com/',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Fundación Dialekta',
        address: 'Calle 76 Cra. 58 #75-158, Floridablanca, Santander, Colombia',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Formalización',
        contactoEMail: 'direccion@dialekta.co',
        contactoWebsite: 'https://www.dialekta.co/',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Fundación Jeshua',
        address: 'Kra 9E # 2C23,Bogotá D.C, Colombia',
        lineaDeGestion: 'Educación e Investigación',
        nivel: 'Formalización',
        contactoEMail: 'direccionejecutiva@fundacionmeis.com',
        contactoWebsite: '',
        departamento: 'Cundinamarca'
      },
      {
        title: 'FUNDAPER',
        address: 'Carrera 15 No. 97-40 Oficina 503, Bogotá D.C, Colombia',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Crecimiento',
        contactoEMail: 'pranainc@gmail.com',
        contactoWebsite: 'https://www.pranaincubadora.org/',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Fundación Soydoy',
        address: 'Carrera 5 No 189 - 22',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Formalización',
        contactoEMail: 'direccion@soydoy.org',
        contactoWebsite: '',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Fundación Forjan ONG',
        address: 'Carrera89 b 76-93',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Formalización',
        contactoEMail: 'forjanon@yahoo.es',
        contactoWebsite: '',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Fundación Pro Derecho a Morir Dignamente',
        address: 'Carrera 11 No. 73-44 Of. 508',
        lineaDeGestion: 'Derechos Civiles Asesoramiento Legal y Política',
        nivel: 'Formalización',
        contactoEMail: 'fsjproyectos@gmail.com',
        contactoWebsite: '',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Fundación Educar Colombia',
        address: 'Cll 55 A Sur No 70/47, Bogotá D.C, Colombia',
        lineaDeGestion: 'Medio Ambiente',
        nivel: 'Crecimiento',
        contactoEMail: 'fundaciongep@gmail.com',
        contactoWebsite: 'https://www.casamuseotequendama.org/',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Fundación para el Apoyo a Población con Necesidades Especiales PONES',
        address: 'Carrera 8 N° 27-28 Sur, Bogotá D.C, Colombia',
        lineaDeGestion: 'No clasificado',
        nivel: 'Formalización',
        contactoEMail: 'info@pocalana.com',
        contactoWebsite: 'https://www.pocalana.org/',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Fundación Acuerdos por la Paz FAPAZ',
        address: 'Calle 38 # 73F - 12, Bogotá D.C, Colombia',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Formalización',
        contactoEMail: '',
        contactoWebsite: '',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Fundación Abuelos Cien Años De Salud y Vida',
        address: 'Diagonal 71c-bis-b sur No 77g -68',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Formalización',
        contactoEMail: 'olga.funacien@gmail.com',
        contactoWebsite: '',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Fundación Formando Sociedad',
        address: 'Calle 8 Sur No 5-10, Bogotá D.C, Colombia',
        lineaDeGestion: 'Servicios Sociales',
        nivel: 'Formalización',
        contactoEMail: 'fundiamet@hotmail.com',
        contactoWebsite: '',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Fundación Alianza Vital',
        address: 'Carrera 69 D No. 24 A - 30, Bogotá D.C, Colombia',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Formalización',
        contactoEMail: '',
        contactoWebsite: '',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Make a Wish Colombia',
        address: 'Calle 60 A Sur no. 78-35,BOGOTÁ D.C.,CUNDINAMARCA,COLOMBIA',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Formalización',
        contactoEMail: '',
        contactoWebsite: '',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Fundación de Educación Integral Percomputo',
        address: 'Calle 55 A No 19 - 11, Bogotá D.C, Colombia',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Despegue',
        contactoEMail: 'fundacionesperanzaypazion@hotmail.com',
        contactoWebsite: '',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Fundación Evolucionar Colombia',
        address: 'Carrera 78 B # 35 A 51 Sur Bloque 9 Apartamento 101, Bogotá D.C, Colombia',
        lineaDeGestion: 'Servicios Sociales',
        nivel: 'Despegue',
        contactoEMail: 'funhuellasalegsa012@yahoo.es',
        contactoWebsite: '',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Fundación Integral Sentir',
        address: 'CARRERA 70 C NO 54-69,BOGOTÁ D.C.,CUNDINAMARCA,COLOMBIA',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Formalización',
        contactoEMail: '',
        contactoWebsite: '',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Corporación Red Somos',
        address: 'Calle 9# 2-73, Bogotá D.C, Colombia.',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Formalización',
        contactoEMail: '',
        contactoWebsite: '',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Fundación Centro para el Reintegro y Atención del Niño CRAN',
        address: 'Tv. 77 #162-06,Bogotá D.C, Colombia',
        lineaDeGestion: 'No clasificado',
        nivel: 'Despegue',
        contactoEMail: 'la.botero@fan.org.co',
        contactoWebsite: 'http://www.fan.org.co/es',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Fundación Los Pisingos',
        address: '',
        lineaDeGestion: 'Servicios Sociales',
        nivel: 'Despegue',
        contactoEMail: 'dirgen@ninandes.org',
        contactoWebsite: 'https://www.ninandes.org/',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Fundación Bella Flor',
        address: 'Cra 27 G N°71G-53, Bogotá D.C, Colombia',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Formalización',
        contactoEMail: '',
        contactoWebsite: '',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Fundación Ayuda por Colombia',
        address: 'Cra. 4 #190B - 02, Bogotá D.C, Colombia',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Formalización',
        contactoEMail: '',
        contactoWebsite: '',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Fundación por un Mañana Mejor',
        address: 'Carrera 2 Este 76 47, Bogotá D.C, Colombia',
        lineaDeGestion: 'No clasificado',
        nivel: 'Formalización',
        contactoEMail: 'fundacionserconsiente.2015@gmail.com',
        contactoWebsite: '',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Asociación Benéfico Cristiana Promotora de Desarrollo Integral A.B.C. PRODEIN',
        address: 'Calle 47 # 36 A 96 Bogotá D.C, Colombia',
        lineaDeGestion: 'Educación e Investigación',
        nivel: 'Crecimiento',
        contactoEMail: '',
        contactoWebsite: 'https://hna.mariaester@prodeincolombia.org/',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Fundación Hogar Integral',
        address: 'Cra 68 C Bis No 37 A 54 Sur, Bogotá D.C, Colombia',
        lineaDeGestion: 'Servicios Sociales',
        nivel: 'Formalización',
        contactoEMail: '',
        contactoWebsite: '',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Corporación Infancia y Desarrollo',
        address: 'Carrera 22 # 143 - 11, Bogotá D.C, Colombia',
        lineaDeGestion: 'No clasificado',
        nivel: 'Despegue',
        contactoEMail: 'gestiondeoportunidades@cid.org.co',
        contactoWebsite: 'http://www.corporacioninfanciaydesarrollo.org/',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Tierra Viva ESAL/ONG Colombia',
        address: 'Calle 123 No 67 91 Lisboa, Bogotá D.C, Colombia',
        lineaDeGestion: 'No clasificado',
        nivel: 'Formalización',
        contactoEMail: '',
        contactoWebsite: '',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Corporación Fondo Biocomercio Colombia',
        address: 'Carrera 15 Calle 90, Bogotá D.C, Colombia',
        lineaDeGestion: 'Medio Ambiente',
        nivel: 'Formalización',
        contactoEMail: 'gusurrea22@hotmail.com',
        contactoWebsite: 'http://www.biocomerciocolombia.com/',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Corporación Enseña por Colombia',
        address: 'Calle 124 # 71-92, Bogotá D.C, Colombia',
        lineaDeGestion: 'Educación e Investigación',
        nivel: 'Despegue',
        contactoEMail: 'n.franco@ensenaporcolombia.org',
        contactoWebsite: 'http://www.ensenaporcolombia.org/',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Corporación Elohim Colombia',
        address: 'CARRERA 112 A BIS 71 C 18 CA 153, BOGOTÁ D.C,COLOMBIA',
        lineaDeGestion: 'No clasificado',
        nivel: 'Formalización',
        contactoEMail: 'lic_javiersanchez@yahoo.es',
        contactoWebsite: '',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Corporación Dunna Alternativas Creativas para la Paz',
        address: 'Carrera 7a # 95-03, Bogotá D.C, Colombia',
        lineaDeGestion: 'Salud',
        nivel: 'Formalización',
        contactoEMail: 'malopez@dunna.org',
        contactoWebsite: 'http://www.dunna.org/',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Corporación Delta Social Humanitaria O.N.G.D.',
        address: 'Avenida Carrera 50 22-03 Sur, Bogotá D.C, Colombia',
        lineaDeGestion: 'Desarrollo y vivienda',
        nivel: 'Formalización',
        contactoEMail: 'corporacioncorporaciondelta@hotmail.com',
        contactoWebsite: 'http://www.corporaciondelta.org.com/',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Corporación de Residencias Universitarias',
        address: 'Cra 33 N 23-51, Bogotá D.C, Colombia',
        lineaDeGestion: 'Servicios Sociales',
        nivel: 'Formalización',
        contactoEMail: 'primoc11@gmail.com',
        contactoWebsite: 'http://www.cru.org.co/',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Corporación de Reciclaje de Bogotá D.C. Corporeciclemos',
        address: 'Cra. 10 No. 20-39, Bogotá D.C, Colombia',
        lineaDeGestion: 'Medio Ambiente',
        nivel: 'Formalización',
        contactoEMail: 'corporeciclemos@hotmail.com',
        contactoWebsite: '',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Corporación de Comunidades Cristianas Scouts',
        address: 'Calle 6 A No.94-A 25 Bogotá D.C, Colombia',
        lineaDeGestion: 'Servicios Sociales',
        nivel: 'Formalización',
        contactoEMail: 'ccsc201407@yahoo.com',
        contactoWebsite: '',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Corporación DAR un Mundo de Negocios y Regalos',
        address: 'Carrera 40 # 1 D 48, Bogotá D.C, Colombia',
        lineaDeGestion: 'Educación e Investigación',
        nivel: 'Formalización',
        contactoEMail: '',
        contactoWebsite: 'http://www.corporaciondar.org/',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Corporación Cultural Nueva Tibabuyes Cultiba',
        address: 'Cra. 143a #135-31, Bogotá D.C, Colombia',
        lineaDeGestion: 'Servicios Sociales',
        nivel: 'Formalización',
        contactoEMail: 'cultiba97@gmail.com',
        contactoWebsite: 'http://www.cultiba.com/',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Corporación Centro Histórico',
        address: 'Carrera 4B Este # 6D - 48, Bogotá D.C, Colombia',
        lineaDeGestion: 'Medio Ambiente',
        nivel: 'Formalización',
        contactoEMail: 'informacion@corporacioncentrohistorico.org',
        contactoWebsite: 'http://www.corporacioncentrohistorico.org/',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Corporación Amor por Colombia',
        address: 'Av Caracas No 16 - 49, Bogotá D.C, Colombia',
        lineaDeGestion: 'No clasificado',
        nivel: 'Formalización',
        contactoEMail: 'direccion@axc.com.co',
        contactoWebsite: '',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Corporación Ágoras',
        address: 'Calle 90 A # 95 - 17, Bogotá D.C, Colombia',
        lineaDeGestion: 'Negocios y Asociaciones Profesionales',
        nivel: 'Formalización',
        contactoEMail: 'derechosagoras@hotmail.com',
        contactoWebsite: 'http://corporacionagora.org/',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Club Deportivo Fútbol Z1C0',
        address: 'Cl 170 No 7-60, Bogotá D.C, Colombia',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Formalización',
        contactoEMail: 'annysan62@hotmail.com',
        contactoWebsite: 'http://www.clubfutbolz1c0.wixsite.com/clubz1c0',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Civitas Foundation',
        address: 'Calle 94 #11-20, Bogotá D.C, Colombia',
        lineaDeGestion: 'Negocios y Asociaciones Profesionales',
        nivel: 'Formalización',
        contactoEMail: 'mparodir23@gmail.com',
        contactoWebsite: '',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Centro de Investigaciones y Servicios Educativos CINSE',
        address: 'Diagonal 46 No. 19-28, Bogotá D.C, Colombia',
        lineaDeGestion: 'Educación e Investigación',
        nivel: 'Formalización',
        contactoEMail: '',
        contactoWebsite: 'directorcinse@lasalle.org.co',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Centro de Bienestar del Anciano Fundación Jeymar',
        address: 'Cra. 8c #187A-59, Bogotá D.C, Colombia',
        lineaDeGestion: 'Servicios Sociales',
        nivel: 'Crecimiento',
        contactoEMail: 'phenao@fundacionjeymar.org',
        contactoWebsite: 'http://www.fundacionjeymar.org/',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Centro de Apoyo Integral y Vida Sana Cavis S.O.S',
        address: 'Cl 70 A 113 A 07, Bogotá D.C, Colombia.',
        lineaDeGestion: 'Servicios Sociales',
        nivel: 'Formalización',
        contactoEMail: 'nislletas@hotmail.com',
        contactoWebsite: '',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Asociación Pan de Vida CER',
        address: 'Av Cra 60 No 57 60, Bogotá D.C, Colombia',
        lineaDeGestion: 'Servicios Sociales',
        nivel: 'Crecimiento',
        contactoEMail: 'pandevidacer@hotmail.com',
        contactoWebsite: 'http://www.pandevidacer.org/',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Asociación Nazareno',
        address: 'Calle 91 No 28C-46, Bogotá D.C, Colombia',
        lineaDeGestion: 'No clasificado',
        nivel: 'Formalización',
        contactoEMail: 'arizalanazareno2011@hotmail.com',
        contactoWebsite: '',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Asociación Juvenil Creando un Sueño',
        address: 'Cll 6A No 6-76, La calera, cundinamarca, Colombia',
        lineaDeGestion: 'Servicios Sociales',
        nivel: 'Formalización',
        contactoEMail: 'ajcuscajica@hotmail.com',
        contactoWebsite: '',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Asociación Folklórica Palos, Cuerdas y Kotizas PCK',
        address: 'Kra. 4 No. 6 - 36 Sur, Bogotá, Colombia',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Formalización',
        contactoEMail: 'pckcultura@yahoo.es',
        contactoWebsite: '',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Asociación Cultural Femenina ACF',
        address: 'Calle 79 # 14-33, Bogotá, Colombia',
        lineaDeGestion: 'No clasificado',
        nivel: 'Formalización',
        contactoEMail: 'direccion@acfemenina.org.co',
        contactoWebsite: 'http://www.acfemenina.org.co/',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Asociación Cristiana de Jóvenes de Bogotá y Cundinamarca',
        address: 'Cra. 16a #28A-33, Bogotá D.C, Colombia',
        lineaDeGestion: 'No clasificado',
        nivel: 'Formalización',
        contactoEMail: 'gloriahidalgo@ymcabogota.org',
        contactoWebsite: 'http://www.ymcabogota.org/',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Asociación Colombianos por la Enfermedad de Huntington',
        address: 'Cra. 77 # 128 A 58, Bogotá D.C, Colombia',
        lineaDeGestion: 'No clasificado',
        nivel: 'Formalización',
        contactoEMail: 'acolpEH@hotmail.com',
        contactoWebsite: 'http://colombianosporlaenfermedaddehuntington.org/',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Asociación Colombiana Médica Estudiantil ACOME',
        address: 'Transversal 14A #112-56, Bogotá D.C, Colombia',
        lineaDeGestion: 'No clasificado',
        nivel: 'Formalización',
        contactoEMail: 'presidente@acome.com.co',
        contactoWebsite: 'http://www.acome.com.co/',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Asociación Colombia Líder',
        address: 'Av. Calle 127 No. 87A-24, Bogotá D.C, Colombia',
        lineaDeGestion: 'No clasificado',
        nivel: 'Formalización',
        contactoEMail: 'dejecutiva@colombialider.org',
        contactoWebsite: '',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Asociación Centro Interdisciplinar de Pensamientos Latinoamericanos Benkos',
        address: 'Carrera 11 A Este No. 1-27, Bogotá D.C, Colombia',
        lineaDeGestion: 'No clasificado',
        nivel: 'Formalización',
        contactoEMail: 'pensamientobenkos@gmail.com',
        contactoWebsite: '',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Asociación Centro de Atención para Niños con Trastornos Sensoriomotores ACONIÑO',
        address: 'Carrera 45 A No. 104 B - 06, Bogotá, Colombia',
        lineaDeGestion: 'Salud',
        nivel: 'Formalización',
        contactoEMail: 'apoyoinclusion@aconino.com',
        contactoWebsite: 'https://www.opcion2.aconino.org/',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Asociación Casanueva',
        address: 'Cr43 A 18 S-135 Int 73, Bogotá D.C, Colombia',
        lineaDeGestion: 'Servicios Sociales',
        nivel: 'Formalización',
        contactoEMail: 'anasoacosta@hotmail.com',
        contactoWebsite: '',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Asociación AURA',
        address: 'Calle 40K Sur No. 74D- 57 Bogotá D.C, Colombia',
        lineaDeGestion: 'Servicios Sociales',
        nivel: 'Formalización',
        contactoEMail: 'asoaura@hotmail.com',
        contactoWebsite: '',
        departamento: 'Cundinamarca'
      },
      {
        title: 'ASIVICO',
        address: 'CL 64 NO. 20 - 56, Bogotá D.C, Colombia',
        lineaDeGestion: 'No clasificado',
        nivel: 'Formalización',
        contactoEMail: 'efrenorjuela@yahoo.es',
        contactoWebsite: '',
        departamento: 'Cundinamarca'
      },
      {
        title: 'ANDAR Colombia',
        address: '66 Carrera 1 Este 19A, La Candelaria, Bogotá, D.C, Colombia',
        lineaDeGestion: 'Educación e Investigación',
        nivel: 'Formalización',
        contactoEMail: 'julian.carrero1@gmail.com',
        contactoWebsite: 'https://andar.uniandes.edu.co/',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Asociación Teatro Elexperimento',
        address: 'Cra 58C #134 A-51, Bogotá D.C, Colombia.',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Despegue',
        contactoEMail: 'elexperimentoteatro@hotmail.com',
        contactoWebsite: 'http://www.elexperimentoteatro.com/',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Fundación Golondrinas',
        address: 'Cra 68 C Bis No 37 A 54 Sur, Bogotá D.C, Colombia',
        lineaDeGestion: 'Servicios Sociales',
        nivel: 'Despegue',
        contactoEMail: 'fundalapu@gmail.com',
        contactoWebsite: '',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Fundación Visión 20/20 Colombia',
        address: 'Calle 34 #51-27, Cachipay, Cundinamarca, Colombia',
        lineaDeGestion: 'Desarrollo y Vivienda',
        nivel: 'Despegue',
        contactoEMail: 'insovivienda@une.net.co',
        contactoWebsite: 'https://www.insovivienda.org/',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Fundación Project Together',
        address: 'Diagonal Colegio Pablo Herrera, Cajicá, Cundinamarca, Colombia',
        lineaDeGestion: 'No clasificado',
        nivel: 'Formalización',
        contactoEMail: 'yannethcastro01@yahoo.es',
        contactoWebsite: 'https://www.fundacionsimeon.org.co/',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Asociación para el Desarrollo Agropecuario y la Convivencia del Norte del Cauca AGROPEC',
        address: 'Calle 23 15 10, Chía, Cundinamarca, Colombia\n',
        lineaDeGestion: 'No clasificado',
        nivel: 'Formalización',
        contactoEMail: 'asociacionagropec@gmail.com',
        contactoWebsite: '',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Fundación para el Niño Sordo ICAL',
        address: 'Carrera 47 A # 118 - 08 Apto 106, Chía, Cundinamarca, Colombia',
        lineaDeGestion: 'No clasificado',
        nivel: 'Formalización',
        contactoEMail: 'mauriciogarcia@proyectodevida.org',
        contactoWebsite: 'https://www.proyectodevida.org/',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Fundación Mi Felicidad',
        address: 'Vereda Fonqueta Sector El Espejo Casa Villa Mil Angeles,CHÍA,CUNDINAMARCA,COLOMBIA',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Formalización',
        contactoEMail: '',
        contactoWebsite: '',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Asociación para el Desarrollo Sostenible y Calidad de Vida ASOCADEVI',
        address: 'Chía, Cundinamarca: Vereda la Balsa, sector el Juncal, finca San Carlos',
        lineaDeGestion: 'No clasificado',
        nivel: 'Formalización',
        contactoEMail: 'amparoforero@gmail.com',
        contactoWebsite: '',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Colectivo de Recuperación Integral CRIAR',
        address: 'Kilómetro 1 1 Via Siberia, Tenjo, Cundinamarca, Colombia',
        lineaDeGestion: 'No clasificado',
        nivel: 'Formalización',
        contactoEMail: 'tsdianamoreno@hotmail.com',
        contactoWebsite: 'http://www.fundacioncriar.com/',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Asociación de Productores de Frutas y Café del Municipio de Tibacuy',
        address: 'Cra 70 C 80-48, Tibacuy, Cundinamarca, Colombia',
        lineaDeGestion: 'Medio Ambiente',
        nivel: 'Formalización',
        contactoEMail: 'asofrucafe@gmail.com',
        contactoWebsite: '',
        departamento: 'Cundinamarca'
      },
      {
        title: 'Asociación Integral Forjadores del Futuro ASOFTSAMARIA',
        address: 'Calle 5 # 4 - 42, Neiva, Huila',
        lineaDeGestion: 'Desarrollo y vivienda',
        nivel: 'Formalización',
        contactoEMail: 'asoftsamaria@gmail.com',
        contactoWebsite: '',
        departamento: 'Huila'
      },
      {
        title: 'Fundación Aprendiendo A Ser Feliz',
        address: 'Calle 10 Nª 20-41',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Formalización',
        contactoEMail: 'fundacionaprendiendoaserfeliz@gmail.com',
        contactoWebsite: '',
        departamento: 'Magdalena'
      },
      {
        title: 'Asociación Empresarial de Proyectos Comunitarios del Norte de Santander',
        address: 'Calle # 2 84 , Bochalema, Norte de Santander, Colombia',
        lineaDeGestion: 'Desarrollo y Vivienda',
        nivel: 'Formalización',
        contactoEMail: 'macve01@yahoo.es',
        contactoWebsite: '',
        departamento: 'Norte de Santander'
      },
      {
        title: 'ASMIRMA',
        address: 'Mz 4 Cs 9 Etapa 3, Dos quebradas, Risaralda, Colombia',
        lineaDeGestion: 'Medio Ambiente',
        nivel: 'Formalización',
        contactoEMail: 'asmirmacolombia@gmail.com',
        contactoWebsite: '',
        departamento: 'Risaralda'
      },
      {
        title: 'Fundación Enfances 2/32 Francia Colombia',
        address: 'Carrera 11bis No 11b 78, Pereira, Risaralda, Colombia',
        lineaDeGestion: 'Salud',
        nivel: 'Despegue',
        contactoEMail: 'fundacionandreayvanessa@gmail.com',
        contactoWebsite: 'https://www.fundacioncasahogarandrea.org/home',
        departamento: 'Risaralda'
      },
      {
        title: 'Corporación Cultural Orquesta de Cuerdas Pulsadas de Risaralda',
        address: 'Calle 29 #3-40, Pereira, Risaralda, Colombia',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Formalización',
        contactoEMail: 'orquestacuerdasrisaralda@gmail.com',
        contactoWebsite: '',
        departamento: 'Risaralda'
      },
      {
        title: 'Asociación Isaral para la Mujer y la Familia',
        address: 'Calle 12 # 23- 232, Pereira, Risaralda.',
        lineaDeGestion: 'No clasificado',
        nivel: 'Formalización',
        contactoEMail: 'asociacionisaral@gmail.com',
        contactoWebsite: '',
        departamento: 'Risaralda'
      },
      {
        title: 'Fundación Recysol',
        address: 'Vereda San Bernardino, Santa Rosa de Cabal, Risaralda, Colombia',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Formalización',
        contactoEMail: 'sndacoljc@gmail.com',
        contactoWebsite: '',
        departamento: 'Risaralda'
      },
      {
        title: 'Fundación Reeducativa Profuturo',
        address: 'Calle 48 N° 19 - 28, Barrancabermeja, Santander,Colombia',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Formalización',
        contactoEMail: 'fundacionreeducativaprofuturo@hotmail.com',
        contactoWebsite: '',
        departamento: 'Santander'
      },
      {
        title: 'Corporación Equivalentes',
        address: 'Calle 39 # 24 -05, Bucaramanga, Santander, Colombia',
        lineaDeGestion: 'Servicios Sociales',
        nivel: 'Formalización',
        contactoEMail: 'corporacionequivalentes@gmail.com',
        contactoWebsite: '',
        departamento: 'Santander'
      },
      {
        title: 'Asociación Niños de Papel',
        address: 'Cl. 48 #18-79, Bucaramanga, Santander, Colombia',
        lineaDeGestion: 'Servicios Sociales',
        nivel: 'Despegue',
        contactoEMail: 'beatrizha@ninosdepapel.org',
        contactoWebsite: 'http://www.ninosdepapel.org/',
        departamento: 'Santander'
      },
      {
        title: 'Fundación Colombo Alemana Volver A Sonreir',
        address: 'Cll 202B # 14 -70',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Formalización',
        contactoEMail: 'fcavolverasonreir@gmail.com',
        contactoWebsite: '',
        departamento: 'Santander'
      },
      {
        title: 'Fundación Formar Estructura',
        address: 'Vereda Potosi Las Delicias,Ibagué, Tolima,Colombia',
        lineaDeGestion: 'Medio Ambiente',
        nivel: 'Despegue',
        contactoEMail: 'fundacioninnovarcampocaribe@gmail.com',
        contactoWebsite: '',
        departamento: 'Tolima'
      },
      {
        title: 'Asociación Fraternidad Internacional de Jóvenes',
        address: 'Carrera 122 No. 70A-23, Argelia, Valle, Colombia',
        lineaDeGestion: 'No clasificado',
        nivel: 'Formalización',
        contactoEMail: 'maryjolanegra@gmail.com',
        contactoWebsite: 'http://www.iyf.org/',
        departamento: 'Valle'
      },
      {
        title: 'Asociación de Mujeres Jefes de Hogar',
        address: 'Calle 6 A No. 71 A -10, Buenaventura, Valle, Colombia',
        lineaDeGestion: 'No clasificado',
        nivel: 'Formalización',
        contactoEMail: 'asomujehtienda@gmail.com',
        contactoWebsite: '',
        departamento: 'Valle'
      },
      {
        title: 'Fundación Verdaderos Héroes',
        address: 'Carrera 61 # 7 - 65, Cali, Valle, Colombia',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Crecimiento',
        contactoEMail: 'fundacionverdaderosheroes@hotmail.com',
        contactoWebsite: '',
        departamento: 'Valle'
      },
      {
        title: 'Fundación Unico',
        address: 'Calle 52 # 3-29,CALI,VALLE,COLOMBIA',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Formalización',
        contactoEMail: '',
        contactoWebsite: '',
        departamento: 'Valle'
      },
      {
        title: 'Fundación Plan de Apoyo Familiar',
        address: 'Carrera 2 oeste # 10-37, Cali, Valle, Colombia',
        lineaDeGestion: 'Servicios Sociales',
        nivel: 'Despegue',
        contactoEMail: 'fernanda.luna@savethechildren.org',
        contactoWebsite: 'https://www.savethechildren.org.co/',
        departamento: 'Valle'
      },
      {
        title: 'Corporación Casa Naranja',
        address: 'Calle 48 No. 18-79, Cali, Valle, Colombia',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Formalización',
        contactoEMail: 'casanaranja4572@gmail.com',
        contactoWebsite: 'https://www.casanaranjateatroalbarrio.com/',
        departamento: 'Valle'
      },
      {
        title: 'Corporación Activando la Consciencia del Amor',
        address: 'Cra 85a # 19-46, Cali, Valle del cauca, Colombia',
        lineaDeGestion: 'Salud',
        nivel: 'Despegue',
        contactoEMail: 'presidencia@laconscienciadelamor.org',
        contactoWebsite: 'https://www.laconscienciadelamor.org/',
        departamento: 'Valle'
      },
      {
        title: 'Cordapa',
        address: 'Km 9,5 Carretera principal Dapa, Yumbo, Valle del cauca, Colombia',
        lineaDeGestion: 'No clasificado',
        nivel: 'Formalización',
        contactoEMail: 'cordapadireccion@gmail.com',
        contactoWebsite: '',
        departamento: 'Valle'
      },
      {
        title: 'Club Activo 20-30 Internacional de Cali',
        address: 'Cl 27 42 B-15 Cali, Valle del cauca, Colombia',
        lineaDeGestion: 'Educación e Investigación',
        nivel: 'Formalización',
        contactoEMail: 'jxhallo@puj.edu.co',
        contactoWebsite: 'https://voces2030colombia.wordpress.com/',
        departamento: 'Valle'
      },
      {
        title: 'Centro Solidaridad la Esperanza CESOLES',
        address: 'Manzana 2 Casa Sector Bermúdez La Esperanza Alto Los Chorros, Cali, Valle.',
        lineaDeGestion: 'Servicios Sociales',
        nivel: 'Formalización',
        contactoEMail: 'cesoles@cesoles.org',
        contactoWebsite: 'http://www.cesoles.org/',
        departamento: 'Valle'
      },
      {
        title: 'Casita de Belén',
        address: 'Cra. 4 #36A, Cali, Valle del Cauca, Colombia',
        lineaDeGestion: 'Servicios Sociales',
        nivel: 'Formalización',
        contactoEMail: 'direccion@casitadebelen.co',
        contactoWebsite: 'http://casitadebelen.co/',
        departamento: 'Valle'
      },
      {
        title: 'Asociación Educativa Edupar',
        address: 'Calle 12 # 4N - 17, Cali, Valle, Colombia',
        lineaDeGestion: 'Servicios Sociales',
        nivel: 'Formalización',
        contactoEMail: 'programaedupar@yahoo.es',
        contactoWebsite: 'https://asoedupar.com/',
        departamento: 'Valle'
      },
      {
        title: 'Asociación de Padres y Amigos de Niños con Sindrome de Down ASOPANID',
        address: 'Cra 16 # 13 -71, Cali, Valle, Colombia',
        lineaDeGestion: 'Servicios Sociales',
        nivel: 'Formalización',
        contactoEMail: 'info@asopanid.com',
        contactoWebsite: 'http://www.asopanid.com/',
        departamento: 'Valle'
      },
      {
        title: 'Asociación de Nuevas Orquestas Caleñas ASONOVAS',
        address: 'Carrera 84 No.45-52, Cali, Valle, Colombia',
        lineaDeGestion: 'Cultura y Recreación',
        nivel: 'Formalización',
        contactoEMail: 'asonovascali@gmail.com',
        contactoWebsite: '',
        departamento: 'Valle'
      },
    ],
    meta: {
      title: 'Aflora - Nuestras organizaciones',
      metaTitle: 'Aflora - Nuestras organizaciones',
      metaDescription: 'Nuestras organizaciones',
      ogTitle: 'Nuestras organizaciones',
      ogDescription: 'Nuestras organizaciones',
      ogImage: '/images/programa-aflora/7-nuestras-organizaciones/aflora_nuestras_organizaciones_og.jpg',
    },
  },
  actualidad: {
    articles: [
      {
        id: 1,
        slug: 'Como-liderar-nuevas-generaciones',
        imgURL: '/images/seccion-actualidad/articulos/como-liderar-nuevas-generaciones.jpeg',
        date: '1 - Septiembre - 2020',
        title: 'Cómo liderar nuevas generaciones',
        shortDescription:
          'Conozca más sobre tres elementos vitales a la hora de enfrentar el reto de liderar nuevas generaciones.',
        description: [
          {
            title: 'Cómo liderar nuevas generaciones',
            text:
              'Por: Valentín Castellanos Mora'+

              'Cómo liderar de forma adecuada siempre será un reto, no solo por las diferencias generacionales si no aún más difícil por la necesidad de personalizar cada conversación y encuentro con un seguidor. Nos hemos hecho constantemente la pregunta de si un líder nace o se hace, la respuesta es simple, el liderazgo no es solo un rasgo de personalidad o un elemento de nuestro carácter, el liderazgo es la suma de comportamientos, momentos, experiencias y aprendizajes que vamos incorporando en nuestra vida. Es así como una conversación de liderazgo hoy podría ser exitosa pero no necesariamente la misma nos serviría 5 años después.<br> <br>'+
              
              
              'Las nuevas generaciones han traído cambios radicales en los conceptos tradicionales de liderazgo, especialmente el que se había basado en un rol o jerarquía. Hoy en día, la comunicación es más directa y menos sumisa por parte de los liderados. Un seguidor expresa con facilidad sus sentimientos y expectativas, espera una conversación influyente y no autoritaria. Es así como el liderazgo actual necesita incorporar negociación, una explicación del sentido de las decisiones y la personalización del mensaje.<br> <br>'+
              
              
              'Como líderes, podemos enfocarnos en 3 elementos vitales para mejorar nuestra relación con liderados de las nuevas generaciones. El primero es el autoconocimiento, un líder que se conoce a sí mismo y entiende sus propias dinámicas, será un líder que genere más confianza con sus liderados. Actualmente el líder debe desprenderse de sus máscaras y mostrar de manera transparente sus cualidades y también sus dificultades. Los equipos están esperando un líder auténtico y consciente de su carácter y el impacto del mismo.<br> <br>'+
              
              
              'El segundo elemento es la empatía, una verdadera intención de conocer al seguidor. Una persona de las nuevas generaciones siente la necesidad de trabajar con personas que son leales, sinceras y confiables. Actualmente, se hace más necesario tener conversaciones sobre expectativas, sueños, objetivos e intereses con nuestros liderados. Conocerlos nos dará la posibilidad de personalizar nuestras estrategias de influencia, entre más información tengamos de nuestros equipos, mayor capacidad de establecer una relación efectiva con nuestros liderados.<br> <br>'+
              
              
              'El tercer elemento es la flexibilidad, la información y los cambios son cada día más abundantes, un líder rígido está expuesto a quedarse rápidamente sin buenos coequiperos. Las nuevas generaciones entienden estas dinámicas y están dispuestos a unirse a un líder que enfrenta la incertidumbre con tranquilidad. Si el líder tiene un propósito claro y lo comparte, le genera seguridad a sus liderados, ellos valoran la convicción de ir por una meta, entendiendo que no siempre se puede llegar por el mismo camino. La flexibilidad también se valora en adaptarse a sus intereses y en entender que el compromiso no se mide por tiempo o permanencia en un lugar, se mide por la pasión y entrega que le ponen a los proyectos.<br> <br>'+
              
              
              'Finalmente, es necesario mencionar la importancia de incluir la tecnología en el liderazgo. Siempre será de mucho valor una conversación cara a cara, sentir la presencia de un seguidor ayuda a entender y leer lo que las palabras no dicen. Sin embargo, hoy en día muchas de las conversaciones se hacen virtualmente, una llamada, un mensaje vía celular o una videollamada, sea cual sea el medio es vital cuidar los códigos. Es por eso que la recomendación es acordar con el equipo, normas y definir estrategias que permitan una comunicación clara y objetiva, impulsando al líder a usar estos medios como herramientas de sus acciones de liderazgo en el día a día.<br> <br>',
            images: [],
            video: '',
          },
         
        ],
        time: '4 minutos',
        tags: [
          {
            title: 'Categoría',
            url: 'javascript:void(0)',
          },
          {
            title: '#EmpredePais',
            url: 'javascript:void(0)',
          },
          {
            title: '#Trabajo',
            url: 'javascript:void(0)',
          },
        ],
      },
      {
        id: 2,
        slug: 'Aprenda-como-conseguir-recursos-para-una-fundacion-social',
        imgURL: '/images/seccion-actualidad/articulos/aprenda_como_conseguir_recursos_para_fundacion.png',
        date: '16 - Enero - 2020',
        title: 'Aprenda como conseguir recursos para una fundación social ',
        shortDescription:
          'Conozca una guía básica para que la laborde conseguir recursos sea más efectiva y su organización logre cumplir sus objetivos. ',
        description: [
          {
            title: '¿Quién ayuda a los que ayudan <br/> en Colombia?',
            text:
              'Hacer que una organización sin ánimo de lucro evolucione y logre cumplir sus metas es una tarea exhaustiva que requiere de un equipo comprometido y una serie de estrategias para lograr recaudar los fondos suficientes. Por eso traemos una guía básica para que esta labor sea más efectiva y su organización logre cumplir sus objetivos.<br> <br>'+

              'De acuerdo al libro “Economía y Gestión de Entidades no Lucrativas”, más que recaudar fondos, se trata de captar recursos, pues en algunos casos y dependiendo del foco de la organización, es mejor recibir donaciones en especie, como por ejemplo medicamentos, equipos, servicios profesionales o personas que quieran donar su tiempo a las múltiples actividades de la organización social.<br> <br>'+
              
              'Antes de entrar en materia, lo primero que usted debe hacer para tener éxito es analizar su visión como organización y sus objetivos a corto, mediano y largo plazo. Esto le permitirá tener un panorama más amplio sobre las necesidades que debe suplir para que su fundación tenga una mayor visibilidad. Lo recomendable es que las acciones que use para captar fondos o recursos estén vinculadas directamente a su misión y que su planteamiento sea mucho más estratégico que creativo. Su éxito no dependerá de ideas geniales o inéditas sino de un trabajo sistemático que contemple algunos trucos y principios esenciales.<br> <br>'+
              
              'La moral en la recaudación<br> <br>'+
              
              'Existen todo tipo de métodos para conseguir recursos. Sin embargo, usted debe encontrar los más acordes para su organización, esto se basa en analizar los valores que su fundación transmite y buscar las actividades propias de estos valores. Para dar un ejemplo, no sería coherente que una organización que se dedica a combatir el alcoholismo realice una reunión para recaudar fondos donde tenga en venta  licores de todo tipo.<br> <br>'+
              
              'Para que su organización no entre en conflictos y no se malinterpreten las estrategias de acción por parte de los miembros de la institución, es recomendable dejar claro cuáles son los límites de recaudación de fondos. Esto le permitirá ser veraz en la información que comparte con su equipo y sus donantes. Proporcióneles todo tipo de documentos que genere confianza, así usted dará un perfil ético que le agregará valor al nombre de su organización sin ánimo de lucro.<br> <br>'+
              
              'Entre los principios que su organización puede manejar, contemple algunos:<br> <br>'+
              
              'Recaudar los fondos o recursos que realmente se necesitan.<br> <br>'+
              'Considerar valiosas y útiles todas las aportaciones que lleguen a la organización.<br> <br>'+
              'Realizar publicidad moderada y equilibrada.<br> <br>'+
              'Ser agradecidos con los donantes.<br> <br>'+
              'Para recibir ayuda primero pídala<br> <br>'+
              
              'Hay muchas organizaciones que creen erróneamente que con informar la labor que realizan van a recibir donantes de todas partes. En este punto es necesario comunicar con precisión y claridad lo que se necesita, “necesito X dinero para X causa” o “necesito X implemento para X propósito”.<br> <br>'+
              
              'También es necesario crear una comunicación “personalizada”. Los donantes deben sentirse identificados personalmente con la solicitud que la organización está generando. Este punto es conocido en publicidad como llamado a una acción, como por ejemplo: “Dona en el número de cuenta X” o “Entra a nuestra página web X y dona” etc.<br> <br>'+
              
              'Usted tiene diferentes grupos a los que puede llegar a pedir fondos. Entre estos están las empresas privadas, las fundaciones subvencionadas, los organismos públicos e incluso las personas naturales. Sin embargo, tenga en cuenta que usted no es la única organización que está pidiendo ayuda, existen cientos que están en la misma labor y que están pidiendo más o menos recursos. Por eso es vital ser claros en lo que se necesita.<br> <br>'+
              
              'Como complemento a esto, muéstrele  al donante lo que se puede lograr con su aporte, esto será clave a la hora de decidir si ayuda o no.<br> <br>'+
              
              'Conseguir recursos también está ligado a las relaciones interpersonales.' + ' Muchas veces se obtienen más donaciones cuando una organización tiene un fuerte vínculo con sus donantes que cuando simplemente da un “gracias” al momento del aporte. Esto es efectivo cuando entre la estrategia se tiene un objetivo a largo plazo. Crear vínculos duraderos y beneficios para las dos partes es una buena táctica y trae consigo aliados duraderos y leales a la causa.<br> <br>'+
              
              'Usted también podrá recibir donaciones haciendo publicidad en medios masivos como televisión, radio, revistas e internet. Pero tenga en cuenta que este esfuerzo es económicamente alto y suele ser más utilizado para generar notoriedad que para obtener donaciones.<br> <br>'+
              
              'Recuerde, para conseguir fondos hay que ser creativamente estratégico. Planifique  y lleve a cabo acciones puntuales e interesantes que tengan el potencial de llamar la atención de sus futuros donantes y retener a los que ya ha ganado.<br> <br>'+
              
              'Inversión Social es un programa de la Fundación Bolívar Davivienda cuyo propósito es cofinanciar proyectos sociales que tengan una problemática identificada y una propuesta de solución pertinente que genere capacidades, empodere comunidades y promueva la sostenibilidad. <br> <br>',
            images: [
              '/images/seccion-actualidad/articulos/aprenda_como_conseguir_recursos_para_fundacion.png'
            ],
            video: '',
          },
         
        ],
        time: '4 minutos',
        tags: [
          {
            title: 'Categoría',
            url: 'javascript:void(0)',
          },
          {
            title: '#Cultivarte',
            url: 'javascript:void(0)',
          },
          {
            title: '#Trabajo',
            url: 'javascript:void(0)',
          },
        ],
      },
      {
        id: 3,
        slug: 'Consejos-para-crear-y-elegir-el-nombre-adecuado-para-su-empresa ',
        imgURL: '/images/seccion-actualidad/articulos/consejos-para-elegir-nombre-empresa.png',
        date: '16 - Enero - 2020',
        title: 'Consejos para crear y elegir el nombre adecuado para su empresa ',
        shortDescription:
          'El nombre es la cara de su marca y debe transmitir aquello a lo que se dedica. Así que, ¿cómo hacer para elegir un nombre memorable? ',
        description: [
          {
            title: 'Consejos para crear y elegir el nombre adecuado para su empresa <br> <br>',
            text:
              'Un millón de cosas pueden salir mal a la hora de elegir un nombre para su organización. Aunque a veces es lo último en lo que pensamos, es uno de los aspectos más importantes del negocio.<br> <br>'+

              'El nombre es la cara de su marca y debe transmitir aquello a lo que se dedica. Así que, ¿cómo hacer para elegir un nombre memorable? Siga algunos de los consejos de Adelyn Zhou, empresaria graduada de Harvard Business School, quien ha creado 5 compañías de tecnología.<br> <br>'+
              
              'Paso 1: Haga una lluvia de ideas y busque inspiración. <br> <br>'+
              
              'En primer lugar, piense en lo que quiere que su empresa sea y lo que espera que su marca represente. Luego, deje volar su imaginación. Piense en sus palabras favoritas (se vale usar un diccionario) o palabras en idiomas extranjeros (aunque pueden haber sido ya tomadas por personas en esos países). Vuelva a la mitología griega, los libros de historia y literatura. Piense en sus frases favoritas, conceptos, etc. Si usted tiene tiempo de sobra, puede salir a caminar y tomar notas sobre los nombres de calles, lugares, u otras empresas que le gusten.<br> <br>'+
              
              'Hoy en día se acostumbra combinar palabras para formar algo nuevo. Por ejemplo, SoHo (un barrio de Nueva York, no la revista), proviene de la unión de las palabras “South of Houston Street” y es ahora una de las zonas más de moda de la ciudad. <br> <br>'+
              
              'Paso 2: Ponga a prueba la pronunciación, ortografía y recordación. <br> <br>'+
              
              'Una vez que tenga una lista de posibles nombres, póngalos a prueba con las siguientes preguntas. Su nombre no tiene que cumplir con todos estos criterios, pero es bueno que cumpla con la mayoría. <br> <br>'+
              
              '• ¿Es fácil de deletrear? ¿Los clientes pueden encontrarlo en internet y compartir su nombre con los demás? <br> <br>'+
              
              '• ¿Es fácil de pronunciar? <br> <br>'+
              
              '• ¿Es fácil de escribir en un teléfono móvil?<br> <br>'+
              
              '• ¿Es fácil de recordar?<br> <br>'+
              
              '• ¿Comienza con una de las primeras letras del alfabeto? (En caso de que se ofrezca en una lista por orden alfabético, automáticamente estará en la parte superior)<br> <br>'+
              
              '• ¿Podría traducirse como algo extraño o inapropiado en un idioma extranjero? (Esto es especialmente importante en el mundo globalizado de hoy. Mire por ejemplo algunos nombres de empresas que probablemente no tuvieron esto en cuenta)<br> <br>'+
              
              '• ¿Es un nombre que no tiene una connotación negativa o controvertida? (Por ejemplo, la empresa de atracción de talento llamada HiredGun, tuvo que cambiarlo porque algunos se molestaban con la palabra palabra “gun” –arma en inglés).<br> <br>'+
              
              'Paso 3: Compruebe la disponibilidad de la URL y el nombre en redes sociales.<br> <br>'+
              
              'Tome los nombres que pasaron bien a través de la anterior lista de verificación y revise si están disponibles como una dirección URL (la dirección de una página web que comienza por www y suele terminar por .com). Una URL sólida, sencilla y fácil de recordar es la clave para un negocio exitoso.<br> <br>'+
              
              'Hoy en día la tarea de encontrar una URL disponible puede ser muy frustrante. Para evitar esto, muchas personas acuden a terminaciones distintas a .com, por ejemplo .io, .es, .vc, .co, entre otros. Puede acudir a Instant Domain Search para buscar fácilmente la disponibilidad de una URL.<br> <br>'+
              
              'Si encuentra una dirección terminada en .com y la compra, es recomendable que compre también los otros dominios comunes asociados con ella, como son .es, .net, o .org. Probablemente no la usará de inmediato, pero si usted se convierte en una gran marca en el futuro, podría terminar necesitándolas.<br> <br>'+
              
              'El siguiente paso es comprobar la disponibilidad de las páginas correspondientes en Facebook, Twitter, Instagram, YouTube y así sucesivamente. Aunque no son algo tan vital como la URL, son importantes, ya que los clientes potenciales pueden acudir a estas redes para encontrar su negocio.<br> <br>'+
              
              'Si los nombres que a usted le gustan están ya todos tomados, considere la opción de añadirle palabras relacionadas con su actividad empresarial como “Grupo”, “Equipo”, “Tecnología”, “Laboratorios”, u otros términos después de su nombre para obtener la URL deseada. Por ejemplo, Facebook comenzó como www.thefacebook.com antes de convertirse en www.facebook.com y Flatiron Health comenzó como www.flatironhealth.com antes de convertirse en www.flatiron.com.'+
              
              'Paso 4: Reciba comentarios.<br> <br>'+
              
              'El siguiente paso es tomar su lista de posibles nombres y preguntarles a amigos y familiares qué opinan para clasificar a sus preferencias. Puede publicar su lista de opciones en Facebook y etiquetar a amigos cuyo consejo considera valioso, o puede crear una encuesta gratuita con SurveyMonkey o Formularios de Google y enviarla por correo electrónico. Si usted tiene una audiencia demográfica específica, encueste personas al azar que se ajuste a su grupo objetivo.<br> <br>'+
              
              'Paso 5: Lleve a cabo una búsqueda de marcas.<br> <br>'+
              
              'Si sus planes son que su empresa llegue a ser una marca importante, sin importar en cuánto tiempo, este es un paso importante para garantizar que no se enredará en cuestiones legales. Puede hacer una búsqueda en la base de datos de la Superintendencia de Industria y Comercio, para ver si otra empresa en el país tiene su mismo nombre. Si alguien más tiene su nombre, no pierda la esperanza. De acuerdo con la legislación sobre marcas, dos empresas pueden tener el mismo nombre si se encuentran en campos muy diferentes y no existe riesgo de confusión. Por ejemplo, hay más de 50 empresas en Estados Unidos que usan las palabras “pop corn” en sus nombres.<br> <br>'+
              
              'Una vez que haya terminado con los pasos anteriores , ¡felicidades! Usted ha encontrado un gran nombre para su nueva compañía.<br> <br>',
            images: [
              '/images/seccion-actualidad/articulos/consejos-para-elegir-nombre-empresa.png'
            ],
            video: '',
          },
        ],
        time: '8 minutos',
        tags: [
          {
            title: 'Categoría',
            url: 'javascript:void(0)',
          },
          {
            title: '#EmprendePais',
            url: 'javascript:void(0)',
          },
          {
            title: '#Trabajo',
            url: 'javascript:void(0)',
          },
        ],
      },
      {
        id: 4,
        slug: 'Las-10-obras-de-musica-clasica-mas-importantes-del-mundo',
        imgURL: '/images/seccion-actualidad/articulos/las-10-obras-de-musica-clasica-mas-importantes.jpeg',
        date: '12 - Octubre - 2018',
        title: 'Las 10 obras de música clásica más importantes del mundo',
        shortDescription:
          'La Filarmónica Joven de Colombia, un programa de la Fundación Bolívar Davivienda, trae a continuación las obras más interpretadas del mundo. ',
        description: [
          {
            title: 'Las 10 obras de música clásica más importantes del mundo',
            text:
              'Los clásicos nunca pasan de moda y las grandes interpretaciones del mundo de todos los tiempos no dejan de interpretarse en las salas de concierto alrededor del globo, por esto la Filarmónica Joven de Colombia, un programa de la Fundación Bolívar Davivienda, trae a continuación las obras más interpretadas del mundo.<br> <br>'+

              'La consultora Bachtrack publicó la lista de las obras de música clásica más representadas basándose en más de 28.000 conciertos programados en escenarios de todo el planeta. Beethoven es, sin duda, el compositor estrella con tres de sus sinfonías situadas en el top 10. Y, en contra de lo que pudiera parecer, solo una obra de Mozart está en la lista, y en novena posición.<br> <br>'+
              
              'Sinfonía número 5 de Beethoven.<br> <br>'+
               'El mismo compositor estrenó su opus 67 en el Theater an der Wien de Viena el 22 de diciembre de 1808. Considerada una de las obras más importantes de todos los tiempos.<br> <br>'+
              
              'Esta obra también fue interpretada por la Filarmónica Joven de Colombia en el cierre de temporada en Bogotá D.C, en el año 2011, dirigida por Matthew Hazelwood.<br> <br>'+
              
              'Mesías de Haendel.<br> <br>'+
              'Compuso su famoso oratorio en sólo tres semanas, en 1741. Narra la vida de Jesús y se suele presentar en navidad, el primero de los tres actos habla del nacimiento de Cristo.<br> <br>'+
              
              '<iframe width="560" height="315" src="https://www.youtube.com/embed/D5T3y4ljlTU" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe><br> <br>'+
              
              'Concierto para violín de Sibelius.<br> <br>'+
              'El finlandés compuso su opus 47. El único que dedicó a un sólo instrumento.'+              
              '<iframe width="560" height="315" src="https://www.youtube.com/embed/YsbrRAgv1b4" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>.<br> <br>'+
              
              'La Filarmónica Joven de Colombia tuvo el placer en el año 2012 en São Paulo, de la mano de la solista Sarah Chang de interpretar esta obra dirigida por Claudio Cruz.<br> <br>'+
              
              'Sinfonía número 5 de Tchaikovsky.<br> <br>'+
              'Tchaikovsky fue el encargado de dirigir el estreno de su sinfonía opus 36 en San Petersburgo en noviembre de 1888, esta composición esta dedicado a la fuerza del destino.<br> <br>'+
              
              'En junio de 2017, en el Kongress Palais – Stadthalle, Festsaal, La Filarmónica Joven de Colombia, tuvo el honor de interpretar esta obra dirigida por Andrés Orozco – Estrada.<br> <br>'+
              
              'Concierto para violín de Mendelssohn.<br> <br>'+
              'El compositor alemán dedicó su obra a este instrumento, no pudo dirigir el estreno de la obra que fue presentada el 13 de marzo de 1845.<br> <br>'+
              
              'La ciudad amurallada de Colombia disfrutó de un espectáculo que tocó las fibras de los amantes de la música clásica en el año 2010, al escuchar la interpretación de esta obra a cargo de la solista Joshua Bell y el Director Carlos Miguel Prieto, integrantes de la Filarmónica Joven de Colombia.<br> <br>'+
              
              'Le puede interesar: Tips para disfrutar mejor un concierto de música sinfónica <a href=" http://bit.ly/2BCATiI2> <br> <br>'+
              
              'Sinfonía número 7 de Beethoven.<br> <br>'+
              'Beethoven consideró esta una de sus mejores obras, el músico dirigió el estreno en diciembre de 1813 en Viena con una orquesta repleta de músicos de primera.<br> <br>'+
              
              '<iframe width="560" height="315" src="https://www.youtube.com/embed/7MqrBauptrE" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe><br> <br>'+
              
              'Sinfonía número 1 de Brahms.<br> <br>'+
              'El compositor romántico tardo 14 años en concluir su primera sinfonía, que se estrenó en noviembre de 1876.<br> <br>'+
              
              '<iframe width="560" height="315" src="https://www.youtube.com/embed/fhHb-62BfpI" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe><br> <br>'+
              
              'En el año 2014, en la ciudad de Barranquilla, los integrantes de la Filarmónica Joven de Colombia, interpretaron en el Teatro Amira de la Rosa esta gran obra dirigida por el Director Andrés Orozco – Estrada.<br> <br>'+
              
              'Sinfonía número 6 de Beethoven.<br> <br>'+
              'La obra se estrenó el 22 de diciembre de 1808 en el mismo concierto en el que se estrenó por primera vez la Quinta sinfonía. Esta obra pretende describir sinfónicamente la naturaleza.<br> <br>'+
              
             '<iframe width="560" height="315" src="https://www.youtube.com/embed/iQGm0H9l9I4" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe><br> <br>'+
              
              'Eine Kleine Nachtmusikde Mozart.<br> <br>'+
              'Una de sus obras más conocidas, estrenada en agosto de 1787. El músico sólo utilizó nueve simples notas repetidas de mil formas.<br> <br>'+
              
              '<iframe width="560" height="315" src="https://www.youtube.com/embed/QZWKUszkbXU" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe><br> <br>'+
              
              'Concierto para Violín de Brahms.<br> <br>'+
              'Una obra de gran dificultad técnica para el instrumento solista.<br> <br>'+
              
              'Sin embargo, para los integrantes de la Filarmónica Joven de Colombia fue un gran reto que asumieron con profesionalismo, dando como resultado un gran concierto en el año 2016, en dónde se destacó la interpretación de esta obra a cargo de la solista Tomaz Liebig y el Director Dante Anzolini.<br> <br>'+
              
               
              
              'Bogotá, D.C.  2011.<br> <br>'+
              
              'Director: Matthew Hazelwood.<br> <br>'+
              
               
              
               
              
              'El mismo compositor estrenó su opus 67 en el Theater an der Wien de Viena el 22 de diciembre de 1808. Considerada una de las obras más importantes de todos los tiempos.<br> <br>'+
              
               
              
               
              
              'São Paulo, Julio 2012<br> <br>'+
              
              'SALA SÂO PAULO<br> <br>'+
              
              'Solista: Sarah Chang<br> <br>'+
              
              'Director: Claudio Cruz<br> <br>'+
              
               
              
               
              
               
              
              'Concierto para violín de Sibelius<br> <br>'+
              'El finlandés compuso su opus 47. El único que dedicó a un sólo instrumento.<br> <br>'+
              
              '– Kasel, Junio 2017<br> <br>'+
              
             'Kongress Palais – Stadthalle, Festsaal<br> <br>'+
              
              'Director: Andrés Orozco-Estrada<br> <br>'+
              
               
              
               
              
              'Sinfonía número 5de Tchaikovsky<br> <br>'+
              'Tchaikovsky fue el encargado de dirigir el estreno de su sinfonía opus 36 en San Petersburgo en noviembre de 1888, esta composición esta dedicado a la fuerza del destino.<br> <br>'+
              
              'En Junio de 2017, en el Kongress Palais – Stadthalle, Festsaal, La Filarmónica Jóven de Colombia, tuvo el honor de interpretar esta obra dirigida por Andrés Orozco – Estrada.<br> <br>'+
              
              'Cartagena, Plaza de la Aduana.<br> <br>'+
              
              'Julio  2010.<br> <br>'+
              
              'Solista Joshua Bell<br> <br>'+
              
              'Director: Carlos Miguel Prieto<br> <br>'+
              
              'Concierto para violínde Mendelssohn<br> <br>'+
              'El compositor alemán dedicó su obra a este instrumento, no pudo dirigir el estreno de la obra que fue presentada el 13 de marzo de 1845.<br> <br>'+
              
              'La ciudad amurallada de Colombia disfrutó de un espectáculo que tocó las fibras de los amantes de la música clásica en el año 2010, al escuchar la interpretación de esta obra a cargo de la solista Joshua Bell y el Director Carlos Miguel Prieto, integrantes de la Filarmónica Jóven de Colombia.<br> <br>'+
              
              'Le puede interesar: Tips para disfrutar mejor un concierto de música sinfónica <a href=" http://bit.ly/2BCATiI"http://bit.ly/2BCATiI </a><br> <br>'+
              
               
              
              'Enero 2018<br> <br>'+
              
              'Sincelejo, Plaza Majagual<br> <br>'+
              
              'Director: Andrew Gourlay<br> <br>'+
              
               
              
               
              
               
              
              'Sinfonía número 7de Beethoven<br> <br>'+
              'Beethoven consideró esta una de sus mejores obras, el músico dirigió el estreno en diciembre de 1813 en Viena con una orquesta repleta de músicos de primera.<br> <br>'+
              
              '<iframe width="560" height="315" src="https://www.youtube.com/embed/7MqrBauptrE" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe><br> <br>'+
              
              'Director: Andrés Orozco-Estrada<br> <br>'+
              
               
              
              '– Barranquilla, 2014<br> <br>'+
              
              'Teatro Amira de la Rosa<br> <br>'+
              
               
              
               
              
              'Sinfonía número 1 de Brahms<br> <br>'+
              'El compositor romántico tardo 14 años en concluir su primera sinfonía, que se estrenó en noviembre de 1876.<br> <br>'+
              
              '<iframe width="560" height="315" src="https://www.youtube.com/embed/fhHb-62BfpI" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe><br> <br>'+
              
              'Manizales, de 2016<br> <br>'+
              
               
              
              'Solista: Tomaz Liebig<br> <br>'+
              
              'Director: Dante Anzolini<br> <br>'+
              
               
              
               
              
              'Concierto para Violínde Brahms<br> <br>'+
              'Una obra de gran dificultad técnica para el instrumento solista.<br> <br>'+
              
              'Sin embargo, para los integrantes de la Filarmónica Jóven de Colombia fue un gran reto que asumieron con profesionalismo, dando como resultado un gran concierto en el año 2016, en dónde se destacó la interpretación de esta obra a cargo de la solista Tomaz Liebig y el Director Dante Anzolini.<br> <br>',
            images: [
              '/images/seccion-actualidad/articulos/las-10-obras-de-musica-clasica-mas-importantes.jpeg'
            ],
            video: '',
          },
        ],
        time: '8 minutos',
        tags: [
          {
            title: 'Categoría',
            url: 'javascript:void(0)',
          },
          {
            title: '#EmprendePais',
            url: 'javascript:void(0)',
          },
          {
            title: '#Trabajo',
            url: 'javascript:void(0)',
          },
        ],
      },
      {
        id: 5,
        slug: 'Empresarios-colombianos',
        imgURL: '/images/seccion-actualidad/articulos/empresarios_colombianos.jpg',
        date: '1 - Octubre - 2018',
        title: 'Empresarios colombianos',
        shortDescription:
          'Lo invitamos a descubrir estos 5 casos de empresarios que con Emprende País, lograron cumplir sus metas en crecimiento, sostenibilidad y rentabilidad. ',
        description: [
          {
            title: 'Empresarios colombianos',
            text:
              '<iframe width="560" height="315" src="https://www.youtube.com/embed/u5KIYGzBKSc" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe><br> <br>'+


              'Cuando las buenas ideas se fortalecen y se acompañan, se convierten en grandes realidades. Lo invitamos a descubrir estos 5 casos de empresarios que con Emprende País, lograron cumplir sus metas en crecimiento, sostenibilidad y rentabilidad.<br> <br>',
            images: [
              '/images/seccion-actualidad/articulos/empresarios_colombianos.jpg'
            ],
            video: '',
          },
        ],
        time: '8 minutos',
        tags: [
          {
            title: 'Categoría',
            url: 'javascript:void(0)',
          },
          {
            title: '#EmprendePais',
            url: 'javascript:void(0)',
          },
          {
            title: '#Trabajo',
            url: 'javascript:void(0)',
          },
        ],
      },
      {
        id: 6,
        slug: 'La-Consagracion-de-la-Primavera',
        imgURL: '/images/seccion-actualidad/articulos/consagracion_primavera.jpg',
        date: '27 - Julio - 2018',
        title: 'La Consagración de la Primavera',
        shortDescription:
          'La Orquesta Filarmónica Joven de Colombia, dirigida por Andrés Orozco Estrada, logra una mágica interpretación coreográfica de La Consagración de la Primavera de Igor Stravinsky. ',
        description: [
          {
            title: 'La Consagración de la Primavera ',
            text:
              'La Orquesta Filarmónica Joven de Colombia, dirigida por Andrés Orozco Estrada, logra una mágica interpretación coreográfica de La Consagración de la Primavera de Igor Stravinsky. Es una realización sin precedentes que hace de esta obra un espectáculo inolvidable.<br> <br>'+
              '<iframe width="560" height="315" src="https://www.youtube.com/embed/joer2oBpsjs" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe><br> <br>',
            images: [
              '/images/seccion-actualidad/articulos/consagracion_primavera.jpg'
            ],
            video: '',
          },
        ],
        time: '8 minutos',
        tags: [
          {
            title: 'Categoría',
            url: 'javascript:void(0)',
          },
          {
            title: '#EmprendePais',
            url: 'javascript:void(0)',
          },
          {
            title: '#Trabajo',
            url: 'javascript:void(0)',
          },
        ],
      },
      {
        id: 7,
        slug: '¿Cuales-deben-ser-los-principios-basicos-de-una-organizacion-social?',
        imgURL: '/images/seccion-actualidad/articulos/cuales-deben-ser-los-principios-basicos.jpeg',
        date: '20 - enero - 2018',
        title: '¿Cuáles deben ser los principios básicos de una organización social? ',
        shortDescription:
          'Conozca los principios básicos para que las organizaciones sociales alcancen el éxito.',
        description: [
          {
            title: '¿Cuáles deben ser los principios básicos de una organización social? ',
            text:
              'Cuáles deben ser los principios básicos de una organización social?<br> <br>'+
              
              'El programa Aflora de la Fundación Bolívar Davivienda apoya y fortalece las organizaciones sociales en su crecimiento, a través de la ruta de crecimiento donde la meta es la maduración, brindando apoyo para que sean más fuertes y alcance sus objetivos propuestos. Para que todo esto sea posible las organizaciones sociales deben tener ciertos principios básicos que ayuden a alcanzar el éxito. Tenga en cuenta los siguientes propósitos para que su organización no se estanque y siga en crecimiento.<br> <br>'+
              
              'Los principios básicos en una organización son:<br> <br>'+
              
              '1. Las organizaciones sociales no deben depender del cambio si no de sus objetivos, es normal que pasen muchos líderes y personas a través de esta, pero no debe perder su norte, son de carácter continuo, siempre con la mirada en el beneficio del grupo social al que representan.<br> <br>'+
              
              '2. Se debe evitar el retraso en los procesos, así mismo la ineficiencia en cada actividad que se realice, esto hará que se disminuyan los costos en las operaciones. Es bueno recordar que, en la gran mayoría de organizaciones, los presupuestos son reducidos por eso la importancia del mejoramiento continuo.<br> <br>'+
              
             '3. Las organizaciones sociales deben tener una comunicación constante entre sus miembros, esto obedece a que pueden presentarse cambios o situaciones inesperadas que afecten a su comunidad. El líder de dicha organización debe liderar estos procesos y hacerlos lo más sencillos posibles.<br> <br>'+
              
              '4. Las organizaciones tienen un fin social, no busca uno personal o empresarial, si tu entidad no está destinada a beneficiar una causa, comunidad o sector específico, no se trata de una organización social, evalúa todo lo que realices, este puede ser el factor por el que muy probablemente las cosas no funcionan.<br> <br>'+
              
              'En la Fundación Bolívar Davivienda a través del programa Aflora apoyamos al crecimiento integral de las organizaciones sociales, desde nuestra experiencia orientamos al trabajo para nunca perder el norte, buscando que las organizaciones mantengan sus principios y alcance el éxito. No pierdas la oportunidad de inscribir tu organización al programa Aflora <br> <br>' ,
            images: [
              '/images/seccion-actualidad/articulos/consagracion_primavera.jpg'
            ],
            video: '',
          },
        ],
        time: '8 minutos',
        tags: [
          {
            title: 'Categoría',
            url: 'javascript:void(0)',
          },
          {
            title: '#EmprendePais',
            url: 'javascript:void(0)',
          },
          {
            title: '#Trabajo',
            url: 'javascript:void(0)',
          },
        ],
      },
      {
        id: 8,
        slug: 'La-diferencia-entre-una-Orquesta-Filarmonica-y-una-Orquesta-Sinfonica',
        imgURL: '/images/seccion-actualidad/articulos/diferencia_sinfonica_filarmonica.jpeg',
        date: '31 - Enero - 2018',
        title: 'La diferencia entre una Orquesta Filarmónica y una Orquesta Sinfónica',
        shortDescription:
          'Conozca qué hay detrás de los términos Orquesta Sinfónica y Orquesta Filarmónica. ',
        description: [
          {
            title: 'La diferencia entre una Orquesta Filarmónica y una Orquesta Sinfónica',
            text:
              'Una ‘orquesta’, según la RAE, es un conjunto de músicos que interpretan obras musicales con diversos instrumentos y que tienen la guía de un director. Específicamente, en el mundo de la música clásica existen dos términos con los que muchos estamos familiarizados: la orquesta sinfónica y la orquesta filarmónica, aunque muchos desconocen en qué se diferencian la una de la otra.<br> <br>'+

              'Para sorpresa del lector, en la actualidad, estos adjetivos se utilizan de manera indistinta y la diferencia se remita al siglo pasado. En la antigüedad, las orquestas filarmónicas estaban compuestas por grupos de personas amantes de la música, que no necesariamente eran músicos profesionales, mientras que las orquestas sinfónicas tenían un patrocinio para la formación profesional de cada uno de los integrantes. Sin embargo, debido a que en la práctica funcionan de la misma manera, los términos terminaron unificándose.<br> <br>'+
              
              'Así pues, una orquesta filarmónica o sinfónica es una agrupación musical de gran tamaño en la que sus integrantes interpretan instrumentos de cuerda como violines, violas, arpa y piano; de vientos como flautas, clarinetes y trompetas y de percusión, como los platillos, el triángulo y los timbales; entre otros instrumentos que dependen de la composición de cada una.<br> <br>'+
              
              'En Colombia, tenemos La Filarmónica Joven de Colombia, o la #LaJoven; un proyecto de la Fundación Bolívar Davivienda que busca contribuir a la construcción social del país a través de giras como la #GiraLibertad, y que, además de abrir la temporada 2018, ha logrado mostrar el talento de sus músicos en diferentes ciudades del país. Conoce más sobre esta iniciativa Aquí.<br> <br>'+
              '<a href="https://www.fundacionbolivardavivienda.org/programas/filarmonicajovendecolombia">https://www.fundacionbolivardavivienda.org/programas/filarmonicajovendecolombia</a>' ,
            images: [
              '/images/seccion-actualidad/articulos/diferencia_sinfonica_filarmonica.jpeg'
            ],
            video: '',
          },
        ],
        time: '8 minutos',
        tags: [
          {
            title: 'Categoría',
            url: 'javascript:void(0)',
          },
          {
            title: '#EmprendePais',
            url: 'javascript:void(0)',
          },
          {
            title: '#Trabajo',
            url: 'javascript:void(0)',
          },
        ],
      },
      {
        id: 9,
        slug: 'Internet-org-la-fundacion-de-Facebook-que-brinda-acceso-gratis-a-la-web',
        imgURL: '/images/seccion-actualidad/articulos/internetorg_la_fundacion_de_facebook.png',
        date: '19 - septiembre - 2016',
        title: 'Internet.org, la fundación de Facebook que brinda acceso gratis a la web',
        shortDescription:
          'Cumple un año internet.org, la iniciativa de Mark Zuckerberg el creador de Facebook, que busca llevar gratuitamente internet a lugares remotos del mundo donde era prácticamente imposible conectarse. ',
        description: [
          {
            title: 'Internet.org, la fundación de Facebook que brinda acceso gratis a la web',
            text:
              'A pesar de estar demostrado que el acceso a internet multiplica las posibilidades de desarrollo de las comunidades, el año pasado solo el 43 por ciento de la población mundial estaba conectada. Esto significa que en nuestro planeta hay todavía más de 3.000 millones de personas que ni siquiera saben qué es la red mundial.<br> <br>'+

              'Acaba de cumplir un año internet.org, la iniciativa de Mark Zuckerberg el creador de Facebook, que busca llevar gratuitamente internet a lugares remotos del mundo donde era prácticamente imposible conectarse. Hasta el momento, han logrado darle acceso a internet a más de 25 millones de personas de países como Colombia, Ghana, Pakistán o Bolivia.<br> <br>'+
              
              'Hace apenas unas pocas semanas logró realizar su primer vuelo experimental en Arizona el avión solar Aquila, desarrollado por el Connectivity Lab de Facebook para llevar internet a zonas donde ni siquiera servicios básicos como el agua o la electricidad han podido llegar.<br> <br>'+
              
              '¿Cómo funciona?'+
              
              'El principal proyecto de Internet.org es la plataforma Free Basics , una aplicación a través de la cual es posible ingresar a sitios como ESPN, Unicef, Facebook y BBC News, sin necesidad de gastar el plan de datos en el celular.<br> <br>'+
              
              'Facebook trabaja constantemente para que más páginas y servicios web estén disponibles a través de Free Basics. Adicionalmente, ha establecido alianzas con operadores de telefonía celular para que permitan a sus usuarios navegar gratis a través de los sitios de aliados.<br> <br>'+
              
              'En Colombia la alianza se estableció hasta el momento con Tigo , y permite a los usuarios acceder gratuitamente a sitios como Wikipedia, Agronet, SuDinero y AccuWeather.<br> <br>' ,
            images: [
              '/images/seccion-actualidad/articulos/internetorg_la_fundacion_de_facebook.png'
            ],
            video: '',
          },
        ],
        time: '8 minutos',
        tags: [
          {
            title: 'Categoría',
            url: 'javascript:void(0)',
          },
          {
            title: '#EmprendePais',
            url: 'javascript:void(0)',
          },
          {
            title: '#Trabajo',
            url: 'javascript:void(0)',
          },
        ],
      },
    ],
    meta: {
      title: 'Actualidad',
      metaTitle: 'Actualidad',
      metaDescription: 'Actualidad Description',
      ogTitle: 'Actualidad',
      ogDescription: 'Actualidad',
      ogImage: '/images/seccion-actualidad/actualidad_og.jpg',
    },
  },
  eventos: {
    events: [
      {
        id: 1,
        slug: 'un-ejemplo-de-evento',
        title: 'Un ejemplo de evento',
        shortDescription:
          'Lugar del evento, Objectively productize value-added care',
        description:
          '<p>Credibly negotiate customized portals vis-a-vis viral platforms. Progressively maximize revolutionary e-services without state of the art infomediaries. Monotonectally unleash exceptional customer service for stand-alone meta-services. Monotonectally negotiate progressive metrics with customized markets. Competently incentivize impactful opportunities after cross functional paradigms.</p>' +
          '<p>promote an expanded array of processes without premium expertise. Authoritatively e-enable team driven results vis-a-vis standardized "outside the box" thinking. Professionally embrace inexpensive manufactured products before inexpensive "outside the box" thinking. Rapidiously myocardinate leading-edge materials and interoperable metrics. Phosfluorescently incentivize team driven partnerships through top-line paradigms. </p>',
        imgURL: '/images/inversion_social.jpg',
        date: '9/Feb/2021',
        time: '8:00 a.m.',
        place: 'Bogotá',
        direction1: 'Carrera 7 # 123-65',
        direction2: 'Hotel Tequendama - Salón Principal',
        direction3: 'Bogotá, Cundinamarca',
        attendURL: 'javascript:void(0)',
      },
      {
        id: 2,
        slug: 'otro-en-cualquier-lugar',
        title: 'Otro en cualquier lugar',
        shortDescription:
          'Lugar del evento, Objectively productize value-added care',
        description:
          '<p>Credibly negotiate customized portals vis-a-vis viral platforms. Progressively maximize revolutionary e-services without state of the art infomediaries. Monotonectally unleash exceptional customer service for stand-alone meta-services. Monotonectally negotiate progressive metrics with customized markets. Competently incentivize impactful opportunities after cross functional paradigms.</p>' +
          '<p>promote an expanded array of processes without premium expertise. Authoritatively e-enable team driven results vis-a-vis standardized "outside the box" thinking. Professionally embrace inexpensive manufactured products before inexpensive "outside the box" thinking. Rapidiously myocardinate leading-edge materials and interoperable metrics. Phosfluorescently incentivize team driven partnerships through top-line paradigms. </p>',
        imgURL: '/images/inversion_social.jpg',
        imgURL: '/images/inversion_social.jpg',
        date: '9/Feb/2021',
        time: '8:00 a.m.',
        place: 'Bogotá',
        direction1: 'Carrera 7 # 123-65',
        direction2: 'Hotel Tequendama - Salón Principal',
        direction3: 'Bogotá, Cundinamarca',
        attendURL: 'javascript:void(0)',
      },
      {
        id: 3,
        slug: 'invitando-a-todos',
        title: 'Invitando a todos',
        shortDescription:
          'Lugar del evento, Objectively productize value-added care',
        description:
          '<p>Credibly negotiate customized portals vis-a-vis viral platforms. Progressively maximize revolutionary e-services without state of the art infomediaries. Monotonectally unleash exceptional customer service for stand-alone meta-services. Monotonectally negotiate progressive metrics with customized markets. Competently incentivize impactful opportunities after cross functional paradigms.</p>' +
          '<p>promote an expanded array of processes without premium expertise. Authoritatively e-enable team driven results vis-a-vis standardized "outside the box" thinking. Professionally embrace inexpensive manufactured products before inexpensive "outside the box" thinking. Rapidiously myocardinate leading-edge materials and interoperable metrics. Phosfluorescently incentivize team driven partnerships through top-line paradigms. </p>',
        imgURL: '/images/inversion_social.jpg',
        imgURL: '/images/inversion_social.jpg',
        date: '9/Feb/2021',
        time: '8:00 a.m.',
        place: 'Bogotá',
        direction1: 'Carrera 7 # 123-65',
        direction2: 'Hotel Tequendama - Salón Principal',
        direction3: 'Bogotá, Cundinamarca',
        attendURL: 'javascript:void(0)',
      },
      {
        id: 4,
        slug: 'title-of-event4',
        title: 'TÍTULO DEL EVENTO',
        shortDescription:
          'Lugar del evento, Objectively productize value-added care',
        description:
          '<p>Credibly negotiate customized portals vis-a-vis viral platforms. Progressively maximize revolutionary e-services without state of the art infomediaries. Monotonectally unleash exceptional customer service for stand-alone meta-services. Monotonectally negotiate progressive metrics with customized markets. Competently incentivize impactful opportunities after cross functional paradigms.</p>' +
          '<p>promote an expanded array of processes without premium expertise. Authoritatively e-enable team driven results vis-a-vis standardized "outside the box" thinking. Professionally embrace inexpensive manufactured products before inexpensive "outside the box" thinking. Rapidiously myocardinate leading-edge materials and interoperable metrics. Phosfluorescently incentivize team driven partnerships through top-line paradigms. </p>',
        imgURL: '/images/inversion_social.jpg',
        imgURL: '/images/inversion_social.jpg',
        date: '9/Feb/2021',
        time: '8:00 a.m.',
        place: 'Bogotá',
        direction1: 'Carrera 7 # 123-65',
        direction2: 'Hotel Tequendama - Salón Principal',
        direction3: 'Bogotá, Cundinamarca',
        attendURL: 'javascript:void(0)',
      },
      {
        id: 5,
        slug: 'title-of-event5',
        title: 'TÍTULO DEL EVENTO',
        shortDescription:
          'Lugar del evento, Objectively productize value-added care',
        description:
          '<p>Credibly negotiate customized portals vis-a-vis viral platforms. Progressively maximize revolutionary e-services without state of the art infomediaries. Monotonectally unleash exceptional customer service for stand-alone meta-services. Monotonectally negotiate progressive metrics with customized markets. Competently incentivize impactful opportunities after cross functional paradigms.</p>' +
          '<p>promote an expanded array of processes without premium expertise. Authoritatively e-enable team driven results vis-a-vis standardized "outside the box" thinking. Professionally embrace inexpensive manufactured products before inexpensive "outside the box" thinking. Rapidiously myocardinate leading-edge materials and interoperable metrics. Phosfluorescently incentivize team driven partnerships through top-line paradigms. </p>',
        imgURL: '/images/inversion_social.jpg',
        imgURL: '/images/inversion_social.jpg',
        date: '9/Feb/2021',
        time: '8:00 a.m.',
        place: 'Bogotá',
        direction1: 'Carrera 7 # 123-65',
        direction2: 'Hotel Tequendama - Salón Principal',
        direction3: 'Bogotá, Cundinamarca',
        attendURL: 'javascript:void(0)',
      },
      {
        id: 6,
        slug: 'title-of-event6',
        title: 'TÍTULO DEL EVENTO',
        shortDescription:
          'Lugar del evento, Objectively productize value-added care',
        description:
          '<p>Credibly negotiate customized portals vis-a-vis viral platforms. Progressively maximize revolutionary e-services without state of the art infomediaries. Monotonectally unleash exceptional customer service for stand-alone meta-services. Monotonectally negotiate progressive metrics with customized markets. Competently incentivize impactful opportunities after cross functional paradigms.</p>' +
          '<p>promote an expanded array of processes without premium expertise. Authoritatively e-enable team driven results vis-a-vis standardized "outside the box" thinking. Professionally embrace inexpensive manufactured products before inexpensive "outside the box" thinking. Rapidiously myocardinate leading-edge materials and interoperable metrics. Phosfluorescently incentivize team driven partnerships through top-line paradigms. </p>',
        imgURL: '/images/inversion_social.jpg',
        imgURL: '/images/inversion_social.jpg',
        date: '9/Feb/2021',
        time: '8:00 a.m.',
        place: 'Bogotá',
        direction1: 'Carrera 7 # 123-65',
        direction2: 'Hotel Tequendama - Salón Principal',
        direction3: 'Bogotá, Cundinamarca',
        attendURL: 'javascript:void(0)',
      },
    ],
    meta: {
      title: 'Eventos',
      metaTitle: 'Eventos',
      metaDescription: 'Eventos Description',
      ogTitle: 'Eventos',
      ogDescription: 'Eventos',
      ogImage: '/images/seccion-eventos/eventos_og.jpg',
    },
  },
  filarmonicaJovenDeColombia: {
    cuentameMasLink: 'javascript:void(0)',
    meta: {
      title: 'Filarmónica joven de colombia',
      metaTitle: 'Filarmónica joven de colombia',
      metaDescription: 'Filarmónica joven de colombia',
      ogTitle: 'Filarmónica joven de colombia',
      ogDescription: 'Filarmónica joven de colombia',
      ogImage: '/images/fundacion_bolivar_davivenda.png',
    },
  },
  filarmonicaJovenDeColombiaConozcanos: {
    testimonios: [
      {
        title: 'Carolina Escalona - Presidenta Fundación Rafael Escalona',
        description:
          'Con Aflora pudimos realizar muchos cambios internos y reconocer qué nos hace falta para llegar a ser una organización autosostenible y con visión.',
      },
      {
        title: 'Marian Tertel - Coordinadora en la Fundación Sole Colombia',
        description:
          'Gracias a Aflora pudimos entender dónde estábamos y hacia dónde teníamos que proyectarnos para cumplir todos nuestros objetivos.',
      },
      {
        title:
          'Isabel Cristina Rengifo - Representante legal de Fundación Mis Corazones Alegres',
        description:
          'Con los ciclos de Aflora logramos armar bien los proyectos y fortalecer los  procesos internos de la fundación',
      },
      {
        title:
          'Isabel Cristina Rengifo - Representante legal de Fundación Mis Corazones Alegres',
        description:
          'Con los ciclos de Aflora logramos armar bien los proyectos y fortalecer los  procesos internos de la fundación',
      },
    ],
    meta: {
      title: 'Filarmónica joven de colombia',
      metaTitle: 'Filarmónica joven de colombia',
      metaDescription: 'Filarmónica joven de colombia',
      ogTitle: 'Filarmónica joven de colombia',
      ogDescription: 'Filarmónica joven de colombia',
      ogImage: '/images/fundacion_bolivar_davivenda.png',
    },
  },
  filarmonicaJovenDeColombiaDesarrollamosTalento: {
    seguirEnlaceLink: 'javascript:void(0)',
    meta: {
      title: 'Filarmónica joven de colombia',
      metaTitle: 'Filarmónica joven de colombia',
      metaDescription: 'Filarmónica joven de colombia',
      ogTitle: 'Filarmónica joven de colombia',
      ogDescription: 'Filarmónica joven de colombia',
      ogImage: '/images/programa-filarmonica-joven/3-desarrollamos-talento/desarrollamostalento_og.jpg',
    },
  },
  filarmonicaJovenDeColombiaOrquesta: {
    integrantes: {
      tabs: [
        {
          name: 'Cobres',
          items: [
            {
              title: 'Miller <br/> Muñoz',
              subtitle: 'Miller Muñoz',
              imgURL: '/images/member_a.png',
            },
            {
              title: 'Daniel <br/> Moreno',
              subtitle: 'Trompeta',
              imgURL: '/images/member_b.jpg',
            },
            {
              title: 'Ana <br/> Molina',
              subtitle: 'Corno francés',
              imgURL: '/images/member_c.jpg',
            },
            {
              title: 'Gloria <br/> Ramirez',
              subtitle: 'Trombon y Tuba',
              imgURL: '/images/member_d.jpg',
            },
            {
              title: 'Miller <br/> Muñoz',
              subtitle: 'Miller Muñoz',
              imgURL: '/images/member_a.png',
            },
            {
              title: 'Daniel <br/> Moreno',
              subtitle: 'Trompeta',
              imgURL: '/images/member_b.jpg',
            },
            {
              title: 'Ana <br/> Molina',
              subtitle: 'Corno francés',
              imgURL: '/images/member_c.jpg',
            },
            {
              title: 'Gloria <br/> Ramirez',
              subtitle: 'Trombon y Tuba',
              imgURL: '/images/member_d.jpg',
            },
          ],
        },
        {
          name: 'Percusión',
          items: [
            {
              title: 'Miller <br/> Muñoz',
              subtitle: 'Miller Muñoz',
              imgURL: '/images/member_a.png',
            },
            {
              title: 'Daniel <br/> Moreno',
              subtitle: 'Trompeta',
              imgURL: '/images/member_b.jpg',
            },
            {
              title: 'Ana <br/> Molina',
              subtitle: 'Corno francés',
              imgURL: '/images/member_c.jpg',
            },
            {
              title: 'Gloria <br/> Ramirez',
              subtitle: 'Trombon y Tuba',
              imgURL: '/images/member_d.jpg',
            },
          ],
        },
        {
          name: 'Cuerdas',
          items: [
            {
              title: 'Miller <br/> Muñoz',
              subtitle: 'Miller Muñoz',
              imgURL: '/images/member_a.png',
            },
            {
              title: 'Daniel <br/> Moreno',
              subtitle: 'Trompeta',
              imgURL: '/images/member_b.jpg',
            },
            {
              title: 'Ana <br/> Molina',
              subtitle: 'Corno francés',
              imgURL: '/images/member_c.jpg',
            },
            {
              title: 'Gloria <br/> Ramirez',
              subtitle: 'Trombon y Tuba',
              imgURL: '/images/member_d.jpg',
            },
          ],
        },
        {
          name: 'Maderas',
          items: [
            {
              title: 'Miller <br/> Muñoz',
              subtitle: 'Miller Muñoz',
              imgURL: '/images/member_a.png',
            },
            {
              title: 'Daniel <br/> Moreno',
              subtitle: 'Trompeta',
              imgURL: '/images/member_b.jpg',
            },
            {
              title: 'Ana <br/> Molina',
              subtitle: 'Corno francés',
              imgURL: '/images/member_c.jpg',
            },
            {
              title: 'Gloria <br/> Ramirez',
              subtitle: 'Trombon y Tuba',
              imgURL: '/images/member_d.jpg',
            },
          ],
        },
      ],
    },
    artistasInvitados: [
      {
        title: 'Johannes <br/> Moser',
        subtitle: 'Violonchelo',
        imgURL: '/images/member_e.png',
      },
      {
        title: 'Gabriel <br/> Ahumada',
        subtitle: 'Flauta',
        imgURL: '/images/member_f.png',
      },
      {
        title: 'Sarah <br/> Chang',
        subtitle: 'Violín',
        imgURL: '/images/member_g.png',
      },
      {
        title: 'Melissa <br/> White',
        subtitle: 'Violín',
        imgURL: '/images/member_h.png',
      },
      {
        title: 'Miller <br/> Muñoz',
        subtitle: 'Miller Muñoz',
        imgURL: '/images/member_a.png',
      },
      {
        title: 'Daniel <br/> Moreno',
        subtitle: 'Trompeta',
        imgURL: '/images/member_b.jpg',
      },
      {
        title: 'Ana <br/> Molina',
        subtitle: 'Corno francés',
        imgURL: '/images/member_c.jpg',
      },
      {
        title: 'Gloria <br/> Ramirez',
        subtitle: 'Trombon y Tuba',
        imgURL: '/images/member_d.jpg',
      },
    ],
    directores: [
      {
        title: 'Andres <br/> Orozco',
        subtitle: 'Director principal',
        imgURL: '/images/member_i.png',
      },
      {
        title: 'Matthew <br/> Sydney',
        subtitle: 'Director invitado',
        imgURL: '/images/member_j.png',
      },
      {
        title: 'Rafael <br/> Payare',
        subtitle: 'Director',
        imgURL: '/images/member_k.png',
      },
      {
        title: 'Alejandro <br/> Posada',
        subtitle: 'Director invitado',
        imgURL: '/images/member_l.png',
      },
      {
        title: 'Miller <br/> Muñoz',
        subtitle: 'Miller Muñoz',
        imgURL: '/images/member_a.png',
      },
      {
        title: 'Daniel <br/> Moreno',
        subtitle: 'Trompeta',
        imgURL: '/images/member_b.jpg',
      },
      {
        title: 'Ana <br/> Molina',
        subtitle: 'Corno francés',
        imgURL: '/images/member_c.jpg',
      },
      {
        title: 'Gloria <br/> Ramirez',
        subtitle: 'Trombon y Tuba',
        imgURL: '/images/member_d.jpg',
      },
    ],
    meta: {
      title: 'Filarmónica joven de colombia orquesta',
      metaTitle: 'Filarmónica joven de colombia orquesta',
      metaDescription: 'Filarmónica joven de colombia orquesta',
      ogTitle: 'Filarmónica joven de colombia orquesta',
      ogDescription: 'Filarmónica joven de colombia orquesta',
      ogImage: '/images/programa-filarmonica-joven/4-orquesta/orquesta_og.jpg',
    },
  },
  voluntariadoFundacionBolivarDavivienda: {
    cuentameMasLink: 'javascript:void(0)',
    meta: {
      title: 'Voluntariado-fundación-Bolivar',
      metaTitle: 'Voluntariado fundación Bolivar',
      metaDescription: 'Voluntariado fundación Bolivar',
      ogTitle: 'Voluntariado fundación Bolivar',
      ogDescription: 'Voluntariado fundación Bolivar',
      ogImage: '/images/programa-voluntariado/1-home-voluntariado/home_og.jpg',
    },
  },
  voluntariadoFundacionBolivarDaviviendaConozcanos: {
    testimonios: [
      {
        title: 'Carolina Escalona - Presidenta Fundación Rafael Escalona',
        description:
          'Con Aflora pudimos realizar muchos cambios internos y reconocer qué nos hace falta para llegar a ser una organización autosostenible y con visión.',
      },
      {
        title: 'Marian Tertel - Coordinadora en la Fundación Sole Colombia',
        description:
          'Gracias a Aflora pudimos entender dónde estábamos y hacia dónde teníamos que proyectarnos para cumplir todos nuestros objetivos.',
      },
      {
        title:
          'Isabel Cristina Rengifo - Representante legal de Fundación Mis Corazones Alegres',
        description:
          'Con los ciclos de Aflora logramos armar bien los proyectos y fortalecer los  procesos internos de la fundación',
      },
      {
        title:
          'Isabel Cristina Rengifo - Representante legal de Fundación Mis Corazones Alegres',
        description:
          'Con los ciclos de Aflora logramos armar bien los proyectos y fortalecer los  procesos internos de la fundación',
      },
    ],
    meta: {
      title: 'Voluntariado fundación Bolivar',
      metaTitle: 'Voluntariado fundación Bolivar',
      metaDescription: 'Voluntariado fundación Bolivar',
      ogTitle: 'Voluntariado fundación Bolivar',
      ogDescription: 'Voluntariado fundación Bolivar',
      ogImage: '/images/programa-voluntariado/2-conozcanos/conozcanos_og.jpg',
    },
  },
  voluntariadoFundacionBolivarDaviviendaDesarrollamosTalento: {
    meta: {
      title: 'Voluntariado fundación Bolivar',
      metaTitle: 'Voluntariado fundación Bolivar',
      metaDescription: 'Voluntariado fundación Bolivar',
      ogTitle: 'Voluntariado fundación Bolivar',
      ogDescription: 'Voluntariado fundación Bolivar',
      ogImage: '/images/programa-voluntariado/3-desarrollamos-talento/desarrollamostalento_og.jpg',
    },
  },
  voluntariadoFundacionBolivarDaviviendaInscripciones: {
    reglamentoLink: 'javascript:void(0)',
    inscribaseLink: 'javascript:void(0)',
    meta: {
      title: 'Voluntariado fundación Bolivar',
      metaTitle: 'Voluntariado fundación Bolivar',
      metaDescription: 'Voluntariado fundación Bolivar',
      ogTitle: 'Voluntariado fundación Bolivar',
      ogDescription: 'Voluntariado fundación Bolivar',
      ogImage: '/images/programa-voluntariado/4-inscripciones/inscripciones_og.jpg',
    },
  },
  emprendePais: {
    convocatoriasLink: 'javascript:void(0)',
    desarrollamosTalentoLink: 'javascript:void(0)',
    meta: {
      title: 'Emprende-país',
      metaTitle: 'Emprende-país',
      metaDescription: 'Emprende país',
      ogTitle: 'Emprende país',
      ogDescription: 'Emprende país',
      ogImage: '/images/fundacion_bolivar_davivenda.png',
    },
  },
  emprendePaisConozcanos: {
    meta: {
      title: 'Emprende país',
      metaTitle: 'Emprende país',
      metaDescription: 'Emprende país',
      ogTitle: 'Emprende país',
      ogDescription: 'Emprende país',
      ogImage: '/images/programa-emprende-pais/1-home-emprende-pais/home_og.jpg',
    },
  },
  emprenderPaisDesarrollamosTalento: {
    meta: {
      title: 'Emprende-país',
      metaTitle: 'Emprende-país',
      metaDescription: 'Emprende país',
      ogTitle: 'Emprende país',
      ogDescription: 'Emprende país',
      ogImage: '/images/programa-emprende-pais/3-desarrollamos-talento/desarrollamostalento_og.jpg',
    },
  },
  emprenderPaisDesarrollamosTalentoConsultores: {
    meta: {
      title: 'Emprende país',
      metaTitle: 'Emprende país',
      metaDescription: 'Emprende país',
      ogTitle: 'Emprende país',
      ogDescription: 'Emprende país',
      ogImage: '/images/programa-emprende-pais/4-consultores/consultores_og.jpg',
    },
  },
  emprenderPaisDesarrollamosTalentoMentores: {
    items: [
      {
        id: 1,
        imgURL: '/images/pic_ae.jpg',
        title: 'Camilo Alban',
        subtitle: 'Finance, Strategy',
        link: 'javascript:void(0)',
        topic: 'Marketing y comunicación',
      },
      {
        id: 2,
        imgURL: '/images/pic_ae.jpg',
        title: 'Camilo Alban',
        subtitle: 'Finance, Strategy',
        link: 'javascript:void(0)',
        topic: 'Marketing y comunicación',
      },
      {
        id: 3,
        imgURL: '/images/pic_ae.jpg',
        title: 'Camilo Alban',
        subtitle: 'Finance, Strategy',
        link: 'javascript:void(0)',
        topic: 'Talento Humano',
      },
      {
        id: 4,
        imgURL: '/images/pic_ae.jpg',
        title: 'Camilo Alban',
        subtitle: 'Finance, Strategy',
        link: 'javascript:void(0)',
        topic: 'Talento Humano',
      },
      {
        id: 5,
        imgURL: '/images/pic_ae.jpg',
        title: 'Camilo Alban',
        subtitle: 'Finance, Strategy',
        link: 'javascript:void(0)',
        topic: 'Talento Humano',
      },
      {
        id: 6,
        imgURL: '/images/pic_ae.jpg',
        title: 'Camilo Alban',
        subtitle: 'Finance, Strategy',
        link: 'javascript:void(0)',
        topic: 'Talento Humano',
      },
    ],
    meta: {
      title: 'Emprende-país',
      metaTitle: 'Emprende-país',
      metaDescription: 'Emprende país',
      ogTitle: 'Emprende país',
      ogDescription: 'Emprende país',
      ogImage: '/images/programa-emprende-pais/5-mentores/mentores_og.jpg',
    },
  },
  emprendePaisConvocatorias: {
    reglamentoLink: 'javascript:void(0)',
    inscribaseLink: 'javascript:void(0)',
    meta: {
      title: 'Emprende-país',
      metaTitle: 'Emprende-país',
      metaDescription: 'Emprende país',
      ogTitle: 'Emprende país',
      ogDescription: 'Emprende país',
      ogImage: '/images/programa-emprende-pais/6-convocatorias/convocatorias_og.jpg',
    },
  },
  emprendePaisPresenciaEnElPais: {
    municipio: [
      {
        title: 'CUNDINAMARCA',
        city: 'Bogota',
        coordinator: 'Coordinador',
        name: 'Juan Manuel Platín',
        email: 'jplatin@fundacionbd.org',
      },
      {
        title: 'VALLE DEL CAUCA',
        city: 'Cali',
        coordinator: 'Coordinador',
        name: 'Margarita Serna',
        email: 'mserna@ccc.org.co',
      },
      {
        title: 'BOLIVAR',
        city: 'Santa Marta',
        coordinator: 'Coordinador',
        name: 'Julieth Ocampo',
        email: 'coordinacion.crecimiento@ccsm.org.co',
      },
      {
        title: 'SANTANDER',
        city: 'Santander',
        coordinator: 'Coordinador',
        name: 'Vanessa Marrugo',
        email: 'epsantander@emprendepais.com',
      },
    ],
    meta: {
      title: 'Emprende-país',
      metaTitle: 'Emprende-país',
      metaDescription: 'Emprende país',
      ogTitle: 'Emprende país',
      ogDescription: 'Emprende país',
      ogImage: '/images/programa-emprende-pais/7-presencia-en-el-pais/presenciapais_og.jpg',
    },
  },
  inversionSocial: {
    desarrollamosTalentoLink: 'javascript:void(0)',
    meta: {
      title: 'Inversión social',
      metaTitle: 'Inversión social',
      metaDescription: 'Inversión social',
      ogTitle: 'Inversión social',
      ogDescription: 'Inversión social',
      ogImage: '/images/fundacion_bolivar_davivenda.png',
    },
  },
  inversionSocialConozcanos: {
    testimonios: [
      {
        title: 'Carolina Escalona - Presidenta Fundación Rafael Escalona',
        description:
          'Con Aflora pudimos realizar muchos cambios internos y reconocer qué nos hace falta para llegar a ser una organización autosostenible y con visión.',
      },
      {
        title: 'Marian Tertel - Coordinadora en la Fundación Sole Colombia',
        description:
          'Gracias a Aflora pudimos entender dónde estábamos y hacia dónde teníamos que proyectarnos para cumplir todos nuestros objetivos.',
      },
      {
        title:
          'Isabel Cristina Rengifo - Representante legal de Fundación Mis Corazones Alegres',
        description:
          'Con los ciclos de Aflora logramos armar bien los proyectos y fortalecer los  procesos internos de la fundación',
      },
      {
        title:
          'Isabel Cristina Rengifo - Representante legal de Fundación Mis Corazones Alegres',
        description:
          'Con los ciclos de Aflora logramos armar bien los proyectos y fortalecer los  procesos internos de la fundación',
      },
    ],
    meta: {
      title: 'Inversión social',
      metaTitle: 'Inversión social',
      metaDescription: 'Inversión social',
      ogTitle: 'Inversión social',
      ogDescription: 'Inversión social',
      ogImage: '/images/fundacion_bolivar_davivenda.png',
    },
  },
  inversionSocialConvocatorias: {
    quieroConocerMasLink: 'javascript:void(0)',
    reglamentoLink: 'javascript:void(0)',
    inscribaseLink: 'javascript:void(0)',
    meta: {
      title: 'Inversión social',
      metaTitle: 'Inversión social',
      metaDescription: 'Inversión social',
      ogTitle: 'Inversión social',
      ogDescription: 'Inversión social',
      ogImage: '/images/fundacion_bolivar_davivenda.png',
    },
  },
  becasAlTalento: {
    cuentameMasLink: 'javascript:void(0)',
    meta: {
      title: 'Becas Al Talento',
      metaTitle: 'Becas Al Talento',
      metaDescription: 'Becas Al Talento Description',
      ogTitle: 'Becas Al Talento',
      ogDescription: 'Becas Al Talento',
      ogImage: '/images/programa-becas-al-talento/1-home-becas/becas_al_talento_og.jpg',
    },
  },
  becasAlTalentoConozcanos: {
    testimonios: [
      {
        title: 'Ana María Núñez - Estudiante de Finanzas y Relaciones Internacionales en la Universidad Externado de Colombia.',
        description:
          'Siento que Becas al Talento ha sido un salvavidas que llegó justo en el momento que más lo necesitaba.',
      },
      {
        title: 'Laura Valdivieso - Encargada del Área de Alianzas Estratégicas Universidad del Rosario.',
        description:
          'Me sorprende que una beca también incluya la formación interna de las personas. Cultivar también habilidades del interior y no sólo ofrecer el apoyo económico. Me parece que están en el camino más justo y más necesario.”',
      },
      {
        title:
          'Juan Sebastián Fernández - Facilitador Programa Mindfulness ',
        description:
          'Considero que es diferente porque los demás programas solo se basan en la parte académica y dejan la parte humana de lado. En cambio en este proyecto buscan que uno sea un ser completo y tenga ambos componentes como eje principal para seguir con el proceso',
      },
      {
        title:
          'Isabel Cristina Rengifo - Representante legal de Fundación Mis Corazones Alegres',
        description:
          'Con los ciclos de Aflora logramos armar bien los proyectos y fortalecer los  procesos internos de la fundación',
      },
    ],
    meta: {
      title: 'Conózcanos',
      metaTitle: 'Conózcanos',
      metaDescription: 'Conózcanos Description',
      ogTitle: 'Conózcanos',
      ogDescription: 'Conózcanos',
      ogImage: '/images/fundacion_bolivar_davivenda.png',
    },
  },
  legales: {
    meta: {
      title: 'Legales',
      metaTitle: 'Legales',
      metaDescription: 'Legales Description',
      ogTitle: 'Legales',
      ogDescription: 'Legales',
      ogImage: '/images/fundacion_bolivar_davivenda.png',
    },
  },
  aboutUs: {
    meta: {
      title: 'About Us',
      metaTitle: 'About Us',
      metaDescription: 'About Us Description',
      ogTitle: 'About Us',
      ogDescription: 'About Us',
      ogImage: '/images/seccion-about/about_us_og.jpg',
    },
    afloraUrl: '/aflora',
    emprendePaisUrl: '/emprende-pais',
    filarmonicaJovendeColombiaUrl: '/filarmonica-joven-de-colombia',
    voluntariadoFamiliaBolívarUrl: '/voluntariado-fundacion-bolivar',
    cultivarteUrl: '/cultivarte',
    inversionSocialUrl: '/inversion-social',
    becasAlTalento: '/becas-al-talento',
  },
  premioNacionalDePeriodismo: {
    meta: {
      title: 'Premio Nacional de Periodismo Simón Bolívar',
      metaTitle: 'Premio Nacional de Periodismo Simón Bolívar',
      metaDescription:
        'Premio Nacional de Periodismo Simón Bolívar Description',
      ogTitle: 'Premio Nacional de Periodismo Simón Bolívar',
      ogDescription: 'Premio Nacional de Periodismo Simón Bolívar',
      ogImage: '/images/seccion-premio/premioNacionalPeriodismo_og.jpg',
    },
    otrosInteresantes: [
      {
        title: 'Título de la noticia',
        tags: 'Categoría #Cultivarte #Apoyo',
        date: 'Septiembre 16 - 2021',
        description:
          'Synergistically redefine high-quality ideas after transparent strategic theme.',
        image: '/images/otros-interesantes.jpeg',
      },
      {
        title: 'Título de la noticia',
        tags: 'Categoría #Cultivarte #Apoyo',
        date: 'Septiembre 16 - 2021',
        description:
          'Synergistically redefine high-quality ideas after transparent strategic theme.',
        image: '/images/otros-interesantes.jpeg',
      },
      {
        title: 'Título de la noticia',
        tags: 'Categoría #Cultivarte #Apoyo',
        date: 'Septiembre 16 - 2021',
        description:
          'Synergistically redefine high-quality ideas after transparent strategic theme.',
        image: '/images/otros-interesantes.jpeg',
      },
      {
        title: 'Título de la noticia',
        tags: 'Categoría #Cultivarte #Apoyo',
        date: 'Septiembre 16 - 2021',
        description:
          'Synergistically redefine high-quality ideas after transparent strategic theme.',
        image: '/images/otros-interesantes.jpeg',
      },
      {
        title: 'Título de la noticia',
        tags: 'Categoría #Cultivarte #Apoyo',
        date: 'Septiembre 16 - 2021',
        description:
          'Synergistically redefine high-quality ideas after transparent strategic theme.',
        image: '/images/otros-interesantes.jpeg',
      },
      {
        title: 'Título de la noticia',
        tags: 'Categoría #Cultivarte #Apoyo',
        date: 'Septiembre 16 - 2021',
        description:
          'Synergistically redefine high-quality ideas after transparent strategic theme.',
        image: '/images/otros-interesantes.jpeg',
      },
    ],
  },
  mapaDelSitio: {
    meta: {
      title: 'Mapa Del Sitio',
      metaTitle: 'Mapa Del Sitio',
      metaDescription: 'Mapa Del Sitio Description',
      ogTitle: 'Mapa Del Sitio',
      ogDescription: 'Mapa Del Sitio',
      ogImage: '/images/fundacion_bolivar_davivenda.png',
    },
  },
  lideresEmprendedores: {
    meta: {
      title: 'Lideres Emprendedores',
      metaTitle: 'Lideres Emprendedores',
      metaDescription: 'Lideres Emprendedores Description',
      ogTitle: 'Lideres Emprendedores',
      ogDescription: 'Lideres Emprendedores',
      ogImage: '/images/fundacion_bolivar_davivenda.png',
    },
    jovenesMusicosUrl: '#',
    gestoresCulturalesUrl: '#',
    costosUrl1: '#',
    costosUrl2: '#',
    mallaCurriculars: [
      {
        title: 'Unidad 1',
        description: 'Empoderamiento y mentalidad reflexiva',
        image: '/images/blog_a.jpg',
      },
      {
        title: 'Unidad 2',
        description: 'Empoderamiento y mentalidad reflexiva',
        image: '/images/blog_a.jpg',
      },
      {
        title: 'Unidad 3',
        description: 'Empoderamiento y mentalidad reflexiva',
        image: '/images/blog_a.jpg',
      },
      {
        title: 'Unidad 4',
        description: 'Empoderamiento y mentalidad reflexiva',
        image: '/images/blog_a.jpg',
      },
      {
        title: 'Unidad 5',
        description: 'Empoderamiento y mentalidad reflexiva',
        image: '/images/blog_a.jpg',
      },
      {
        title: 'Unidad 6',
        description: 'Empoderamiento y mentalidad reflexiva',
        image: '/images/blog_a.jpg',
      },
    ],
  },
  redDeAngeles: {
    meta: {
      title: 'Red De Angeles',
      metaTitle: 'Red De Angeles',
      metaDescription: 'Red De Angeles Description',
      ogTitle: 'Red De Angeles',
      ogDescription: 'Red De Angeles',
      ogImage: '/images/fundacion_bolivar_davivenda.png',
    },
    perfilesEmpresasUrl: 'https://fundacionbolivardaviviendaorg.force.com/comunidadfbd/s/?language=es_CO&programa=Red+de+angeles+inversionistas&grupo=Empresas+Red+de+Angeles',
    perfilesInversionistaUrl: 'https://fundacionbolivardaviviendaorg.force.com/comunidadfbd/s/?language=es_CO&programa=Red+de+angeles+inversionistas&grupo=Inversionistas',
    requisitos: [
      {
        image: '/images/icon-user-group.png',
        description: 'Su emprendimiento no destruye valor social o ambiental',
      },
      {
        image: '/images/icon-user-group.png',
        description: 'Su emprendimiento no destruye valor social o ambiental',
      },
      {
        image: '/images/icon-user-group.png',
        description: 'Su emprendimiento no destruye valor social o ambiental',
      },
      {
        image: '/images/icon-user-group.png',
        description: 'Su emprendimiento no destruye valor social o ambiental',
      },
      {
        image: '/images/icon-user-group.png',
        description: 'Su emprendimiento no destruye valor social o ambiental',
      },
      {
        image: '/images/icon-user-group.png',
        description: 'Su emprendimiento no destruye valor social o ambiental',
      },
      {
        image: '/images/icon-user-group.png',
        description: 'Su emprendimiento no destruye valor social o ambiental',
      },
    ],
    testimonios: [
      {
        title: 'Carolina Escalona - Presidenta Fundación Rafael Escalona',
        description:
          'Con Aflora pudimos realizar muchos cambios internos y reconocer qué nos hace falta para llegar a ser una organización autosostenible y con visión.',
      },
      {
        title: 'Marian Tertel - Coordinadora en la Fundación Sole Colombia',
        description:
          'Gracias a Aflora pudimos entender dónde estábamos y hacia dónde teníamos que proyectarnos para cumplir todos nuestros objetivos.',
      },
      {
        title:
          'Isabel Cristina Rengifo - Representante legal de Fundación Mis Corazones Alegres',
        description:
          'Con los ciclos de Aflora logramos armar bien los proyectos y fortalecer los  procesos internos de la fundación',
      },
      {
        title:
          'Isabel Cristina Rengifo - Representante legal de Fundación Mis Corazones Alegres',
        description:
          'Con los ciclos de Aflora logramos armar bien los proyectos y fortalecer los  procesos internos de la fundación',
      },
      {
        title:
          'Isabel Cristina Rengifo - Representante legal de Fundación Mis Corazones Alegres',
        description:
          'Con los ciclos de Aflora logramos armar bien los proyectos y fortalecer los  procesos internos de la fundación',
      },
      {
        title:
          'Isabel Cristina Rengifo - Representante legal de Fundación Mis Corazones Alegres',
        description:
          'Con los ciclos de Aflora logramos armar bien los proyectos y fortalecer los  procesos internos de la fundación',
      },
      {
        title:
          'Isabel Cristina Rengifo - Representante legal de Fundación Mis Corazones Alegres',
        description:
          'Con los ciclos de Aflora logramos armar bien los proyectos y fortalecer los  procesos internos de la fundación',
      },
      {
        title:
          'Isabel Cristina Rengifo - Representante legal de Fundación Mis Corazones Alegres',
        description:
          'Con los ciclos de Aflora logramos armar bien los proyectos y fortalecer los  procesos internos de la fundación',
      },
    ],
  },
  escuelaDeDirectoresLanding: {
    meta: {
      title: 'Escuela De Directores Landing',
      metaTitle: 'Escuela De Directores Landing',
      metaDescription: 'Escuela De Directores Landing Description',
      ogTitle: 'Escuela De Directores Landing',
      ogDescription: 'Escuela De Directores Landing',
      ogImage: '/images/fundacion_bolivar_davivenda.png',
    },
    metodoDelCasos: [
      {
        title: 'Unidad 1',
        description: 'Método del caso',
        image: '/images/blog_a.jpg',
      },
      {
        title: 'Unidad 2',
        description: 'Método del caso',
        image: '/images/blog_a.jpg',
      },
      {
        title: 'Unidad 3',
        description: 'Método del caso',
        image: '/images/blog_a.jpg',
      },
      {
        title: 'Unidad 3',
        description: 'Método del caso',
        image: '/images/blog_a.jpg',
      },
    ],
  },
  becasConozcanos: {
    testimonios: [
      {
        title: 'Carolina Escalona - Presidenta Fundación Rafael Escalona',
        description:
          'Con Aflora pudimos realizar muchos cambios internos y reconocer qué nos hace falta para llegar a ser una organización autosostenible y con visión.',
      },
      {
        title: 'Marian Tertel - Coordinadora en la Fundación Sole Colombia',
        description:
          'Gracias a Aflora pudimos entender dónde estábamos y hacia dónde teníamos que proyectarnos para cumplir todos nuestros objetivos.',
      },
      {
        title:
          'Isabel Cristina Rengifo - Representante legal de Fundación Mis Corazones Alegres',
        description:
          'Con los ciclos de Aflora logramos armar bien los proyectos y fortalecer los  procesos internos de la fundación',
      },
      {
        title:
          'Isabel Cristina Rengifo - Representante legal de Fundación Mis Corazones Alegres',
        description:
          'Con los ciclos de Aflora logramos armar bien los proyectos y fortalecer los  procesos internos de la fundación',
      },
    ],
    meta: {
      title: 'Becas Conózcanos',
      metaTitle: 'Becas Conózcanos',
      metaDescription: 'Becas Conózcanos Description',
      ogTitle: 'Becas Conózcanos',
      ogDescription: 'Becas Conózcanos',
      ogImage: '/images/fundacion_bolivar_davivenda.png',
    },
  },
  becasDesarrollamosTalento: {
    meta: {
      title: 'Becas Desarrollamos Talento',
      metaTitle: 'Desarrollamos Talento',
      metaDescription: 'Desarrollamos Talento Description',
      ogTitle: 'Desarrollamos Talento',
      ogDescription: 'Desarrollamos Talento',
      ogImage: '/images/programa-becas-al-talento/3-desarrollamos-talento/desarrollamos-talento_og.jpg',
    },
  },
  becasRequisitos: {
    meta: {
      title: 'Requisitos',
      metaTitle: 'Requisitos',
      metaDescription: 'Requisitos Description',
      ogTitle: 'Requisitos',
      ogDescription: 'Requisitos',
      ogImage: '/images/programa-becas-al-talento/4-requisitos/requisitos_og.jpg',
    },
  },
  cultivarteHome: {
    desarrollamosTalento: 'javascript:void(0)',
    meta: {
      title: 'Cultivarte',
      metaTitle: 'Cultivarte',
      metaDescription: 'Cultivarte Description',
      ogTitle: 'Cultivarte',
      ogDescription: 'Cultivarte',
      ogImage: '/images/programa-cultivarte/1-home-cultivarte/cultivarte_home_og.jpg',
    },
  },
  cultivarteConozcanos: {
    meta: {
      title: 'Cultivarte - Conozcanos',
      metaTitle: 'Cultivarte - Conozcanos',
      metaDescription: 'Cultivarte Conozcanos Description',
      ogTitle: 'Cultivarte Conozcanos',
      ogDescription: 'Cultivarte Conozcanos',
      ogImage: '/images/programa-cultivarte/2-conozcanos/conozcanos_og.jpg',
    },
    testimonios: [
      {
        title: 'Sara Castro - 10',
        description:
          'La educación financiera es como las matemáticas, solo que aquí juegas con los números y aprendes a ahorrar jugando como si fueras un banco de verdad',
      },
      {
        title: 'Dilan Cabezas - 9',
        description:
          'Los procesos de lectura, como ‘viajes de papel’, son la forma en la que puedes vivir el libro con juegos, sentirlo y que lo puedas recordar siempre',
      },
      {
        title: 'Mariangel Molina - 7',
        description:
          'Los procesos de lectura, como ‘viajes de papel’, son la forma en la que puedes vivir el libro con juegos, sentirlo y que lo puedas recordar siempre',
      },
      {
        title: 'Sara Castro - 11',
        description:
          'La educación financiera es como las matemáticas, solo que aquí juegas con los números y aprendes a ahorrar jugando como si fueras un banco de verdad',
      },
      {
        title: 'Dilan Cabezas - 10',
        description:
          'Los procesos de lectura, como ‘viajes de papel’, son la forma en la que puedes vivir el libro con juegos, sentirlo y que lo puedas recordar siempre',
      },
      {
        title: 'Mariangel Molina - 8',
        description:
          'Los procesos de lectura, como ‘viajes de papel’, son la forma en la que puedes vivir el libro con juegos, sentirlo y que lo puedas recordar siempre',
      },
      {
        title: 'Sara Castro - 12',
        description:
          'La educación financiera es como las matemáticas, solo que aquí juegas con los números y aprendes a ahorrar jugando como si fueras un banco de verdad',
      },
      {
        title: 'Dilan Cabezas - 11',
        description:
          'Los procesos de lectura, como ‘viajes de papel’, son la forma en la que puedes vivir el libro con juegos, sentirlo y que lo puedas recordar siempre',
      },
      {
        title: 'Mariangel Molina - 9',
        description:
          'Los procesos de lectura, como ‘viajes de papel’, son la forma en la que puedes vivir el libro con juegos, sentirlo y que lo puedas recordar siempre',
      },
    ],
  },
  cultivarteDesarrollamosTalento: {
    meta: {
      title: 'Cultivarte Desarrollamos Talento',
      metaTitle: 'Cultivarte Desarrollamos Talento',
      metaDescription: 'Cultivarte Desarrollamos Talento Description',
      ogTitle: 'Cultivarte Desarrollamos Talento',
      ogDescription: 'Cultivarte Desarrollamos Talento',
      ogImage: '/images/programa-cultivarte73-desarrollamos-talento/desarrollamostalento_og.jpg',
    },
  },
  cultivarteDondeEstamosUbicados: {
    meta: {
      title: 'Cultivarte Donde Estamos Ubicados',
      metaTitle: 'Cultivarte Donde Estamos Ubicados',
      metaDescription: 'Cultivarte Donde Estamos Ubicados Description',
      ogTitle: 'Cultivarte Donde Estamos Ubicados',
      ogDescription: 'Cultivarte Donde Estamos Ubicados',
      ogImage: '/images/programa-cultivarte/5-ubicacion/dondeestamos_og.jpg',
    },
  },
  cultivarteZonaLudica: {
    meta: {
      title: 'Cultivarte - Zona Ludica',
      metaTitle: 'Cultivarte - Zona Ludica',
      metaDescription: 'Cultivarte Zona Ludica Description',
      ogTitle: 'Cultivarte Zona Ludica',
      ogDescription: 'Cultivarte Zona Ludica',
      ogImage: '/images/programa-cultivarte/4-zona-ludica/zonaludica_og.jpg',
    },
    plataformas: [
      {
        title: 'LOS ZÚPER',
        image: '/images/programa-cultivarte/4-zona-ludica/logos_plataformas/zl_Los_Zuper_Logo.png',
        detailUrl: 'javascript:void(0)',
      },
      {
        title: 'CODE COMBAT',
        image: '/images/programa-cultivarte/4-zona-ludica/logos_plataformas/zl_Logo_CodeCombat.png',
        detailUrl: 'javascript:void(0)',
      },
      {
        title: 'BIBLIOTECA VIVA',
        image: '/images/programa-cultivarte/4-zona-ludica/logos_plataformas/zl_Logo_BibliotecaViva.png',
        detailUrl: 'javascript:void(0)',
      },
      {
        title: 'SEGURÓPOLIS KIDS',
        image: '/images/programa-cultivarte/4-zona-ludica/logos_plataformas/zl_Logo_Seguropolis.png',
        detailUrl: 'javascript:void(0)',
      },
      {
        title: 'MONETARIUM EL UNIVERSO DE LOS NIÑOS',
        image: '/images/programa-cultivarte/4-zona-ludica/logos_plataformas/zl_Logos-AeF-Monetarium.png',
        detailUrl: 'javascript:void(0)',
      },
      {
        title: 'CASITA DE ARTISTAS',
        image: '/images/programa-cultivarte/4-zona-ludica/logos_plataformas/zl_Logo_Casita_de_Artistas.png',
        detailUrl: 'javascript:void(0)',
      },
      {
        title: 'DISCOVERY EN LA ESCUELA',
        image: '/images/programa-cultivarte/4-zona-ludica/logos_plataformas/zl_Logo_Discovery.png',
        detailUrl: 'javascript:void(0)',
      },
      {
        title: 'GUARDIANES DE LA MÚSICA',
        image: '/images/programa-cultivarte/4-zona-ludica/logos_plataformas/zl_Logo_Guardianes_de_la_musica.png',
        detailUrl: 'javascript:void(0)',
      },
    ],
  },
  donaciones: {
    link1: 'https://www.zonapagos.com/formulario/',
    link2: 'https://www.zonapagos.com/formulario/',
    link3: 'https://www.zonapagos.com/formulario/',
    meta: {
      title: 'Donaciones',
      metaTitle: 'Donaciones',
      metaDescription: 'Donaciones Description',
      ogTitle: 'Donaciones',
      ogDescription: 'Donaciones',
      ogImage: '/images/fundacion_bolivar_davivenda.png',
    },
  }
}
